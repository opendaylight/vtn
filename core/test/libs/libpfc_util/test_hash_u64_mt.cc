...
/*
 * tests for u64hash basic operations by multi-threads
 */
...
thr_target (void *arg)
{
    ...
    uint64_t       target_key = test_data->target_key;
    ...
    while (cur_usec() - start_usec < loop_usec) {
        ...
        switch (target_op) {
            case HASH_OP_GET: {
                EXPECT_EQ(0, pfc_u64hash_get(hash, target_key, &v));
            ...
            case HASH_OP_PUT: {
                pfc_cptr_t v = NULL;
                EXPECT_EQ(0, pfc_u64hash_put(hash, target_key, target_val, 0));
                EXPECT_EQ(0, pfc_u64hash_get(hash, target_key, &v));
                EXPECT_TRUE(target_val == v);
                EXPECT_EQ(0, pfc_u64hash_remove(hash, target_key));
                break;
            }
            case HASH_OP_UPDATE: {
                pfc_cptr_t v = NULL;
                EXPECT_EQ(0, pfc_u64hash_update(hash, target_key, target_val, 0));
                EXPECT_EQ(0, pfc_u64hash_get(hash, target_key, &v));
                EXPECT_TRUE(target_val == v);
                break;
            }
            case HASH_OP_REMOVE: {
                pfc_cptr_t v = NULL;
                EXPECT_EQ(0, pfc_u64hash_update(hash, target_key, &v, 0));
                EXPECT_EQ(0, pfc_u64hash_remove(hash, target_key));
                EXPECT_EQ(ENOENT, pfc_u64hash_get(hash, target_key, NULL));
                break;
            }
...
thr_side (void *arg)
{
    ...
    while (cur_usec() - start_usec < loop_usec) {
        ++count;
        uint64_t side_key = key[count % nentries];
        pfc_cptr_t side_val =
        ...
        switch (side_op) {
            case HASH_OP_GET: {
                pfc_cptr_t v = NULL;
                int err = pfc_u64hash_get(hash, side_key, &v);
            ...
            case HASH_OP_PUT: {
                int err = pfc_u64hash_put(hash, side_key, side_val, 0);
            ...
            case HASH_OP_UPDATE: {
                int err = pfc_u64hash_update(hash, side_key, side_val, 0);
                EXPECT_TRUE( err == 0 );
                // update operation is not target to record `nents_delta'
                break;
            }
            case HASH_OP_REMOVE: {
                int err = pfc_u64hash_remove(hash, side_key);
...
TEST_HASH_MT_OP(get,
                EXPECT_EQ(0, pfc_u64hash_update(test_data->hash,
...
TEST_HASH_MT_OP(put,
                pfc_u64hash_remove(test_data->hash,
                                   test_data->target_key)
                );
...
{
    EXPECT_EQ(0, pfc_u64hash_create(&(test_data->hash),
                                    NULL,
                                    test_data->nbuckets,
                                    0));
}
...
test_data_set_entries (test_data_t *test_data, size_t size)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        int err = pfc_u64hash_put(hash, key[i], val[0][i], 0);
        if (err != 0) abort();
    }
}
