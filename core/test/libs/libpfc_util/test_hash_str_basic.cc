...
TEST(hash, str_create_zero_bucket)
{
    pfc_hash_t hash;
    int err = pfc_strhash_create(&hash, NULL, 0, 0);
    EXPECT_NE(0, err);
}


TEST(hash, str_create_over_buckets)
{
    pfc_hash_t hash;
    int err = pfc_strhash_create(&hash, NULL, PFC_HASH_MAX_NBUCKETS + 1, 0);
    EXPECT_NE(0, err);
}


TEST(hash, str_create_good_buckets_1)
{
    ...
    {
        pfc_hash_t hash;
        EXPECT_EQ(0, pfc_strhash_create(&hash, NULL, nbuckets, 0));
...
TEST(hash, str_create_good_buckets_2)
{
...
TEST(hash, str_create_good_buckets_3)
{
    for (size_t nbuckets = 1;
         nbuckets <= PFC_HASH_MAX_NBUCKETS;
...
TEST(hash, str_clear)
{
    ...
    pfc_hash_t hash;
    int err = pfc_strhash_create(&hash, NULL, nbuckets, 0);
    EXPECT_EQ(0, err);
    ...
    // clear non-empty hash
    {
        ...
        for (int i = 0; i < nentries; i++) {
            pfc_cptr_t val = NULL;
            int get_err = pfc_strhash_get(hash, test_data->key[i], &val);
            EXPECT_EQ(ENOENT, get_err);
            int get_err2 = pfc_strhash_get(hash, test_data->key[i], NULL);
...
TEST(hash, str_put_update2)
{
    // This test complements `TEST(hash, str_put_update)' which
    ...
    // Do put unregistered (NULL, NULL) entry  (It should success)
    {
        ...
        // do put
        EXPECT_EQ(0, pfc_strhash_put(hash, NULL, NULL, 0));
    ...
    // Do put egistered (NULL, NULL) entry  (It should fail)
    {
        ...
        // do put
        EXPECT_EQ(EEXIST, pfc_strhash_put(hash, NULL, NULL, 0));
    ...
    // Do update unregistered (NULL, NULL) entry  (It should success)
    {
        ...
        // do put
        EXPECT_EQ(0, pfc_strhash_update(hash, NULL, NULL, 0));
...
TEST(hash, str_get)
{
    ...
    for (int i = 0; i < nentries; ++i) {
        ...
        {
            int get_err = pfc_strhash_get(hash, key, NULL);
            EXPECT_EQ(ENOENT, get_err);
        }
        {
            pfc_cptr_t get_val = NULL;
            int get_err = pfc_strhash_get(hash, key, &get_val);
            EXPECT_EQ(ENOENT, get_err);
...
TEST(hash, str_get_uint_val)
{
    ...
    for (int i = 0; i < nentries; ++i) {
        ...
        // and it should fail
        {
            ...
            int get_err = pfc_strhash_get(hash, key, get_valp);
            EXPECT_EQ(ENOENT, get_err);
        ...
        // put uint value as pointer
        {
            ...
            int put_err = pfc_strhash_put(hash, key, put_val, 0);
            EXPECT_EQ(0, put_err);
        }
    ...
    for (int i = 0; i < nentries; ++i) {
        uint_val_t get_val[3] = { check_val, 0, check_val };
        pfc_cptr_t *get_valp = (pfc_cptr_t *)(uintptr_t)(&get_val[1]);
        int get_err = pfc_strhash_get(hash, test_data->key[i], get_valp);
        EXPECT_EQ(0, get_err);
        EXPECT_EQ(uint_val[i], get_val[1]);
        EXPECT_EQ(check_val, get_val[0]);
        EXPECT_EQ(check_val, get_val[2]);
...
TEST(hash, str_remove)
{
    ...
    for (int i = 0; i < nentries; ++i) {
        const char *key = test_data->key[i];
        {
            int remove_err = pfc_strhash_remove(hash, key);
    ...
    for (int i = 0; i < nentries; ++i) {
        const char *key = test_data->key[i];
        {
            int get_err1 = pfc_strhash_get(hash, key, NULL);
            EXPECT_EQ(0, get_err1);

            int remove_err = pfc_strhash_remove(hash, key);
            EXPECT_EQ(0, remove_err);

            int get_err2 = pfc_strhash_get(hash, key, NULL);
            EXPECT_EQ(ENOENT, get_err2);
        }
    }
...
test_get_size (int nbuckets, int nentries)
{
    ...
    for (int i = 0; i < nentries; ++i) {
        ...
        int put_err = pfc_strhash_put(hash, test_data->key[i],
                                      test_data->val[0][i], 0);
        EXPECT_EQ(0, put_err);
    }
    ...
    for (int i = 0; i < nentries; ++i) {
        ...
        int remove_err = pfc_strhash_remove(hash, test_data->key[i]);
        EXPECT_EQ(0, remove_err);
    }
...
test_get_set_capacity (int nbuckets, int nentries, ssize_t *new_caps)
{
    ...
    for (ssize_t *next_cap = new_caps; *next_cap > 0; ++next_cap) {
        ...
        for (int i = 0; i < test_data->nentries; ++i) {
            pfc_cptr_t get_val = NULL;
            int get_err = pfc_strhash_get(hash, test_data->key[i], &get_val);
            EXPECT_EQ(0, get_err);
...
TEST(hash, str_get_set_capacity_1)
{
...
TEST(hash, str_get_set_capacity_2)
{
...
TEST(hash, str_get_set_capacity_3)
{
...
TEST(hash, str_set_capacity_error)
{
    // init hash
    int nbuckets = 31;
    pfc_hash_t hash;
...
TEST(hash, str_iterator_get_next_put)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
    ...
    for (int i = -1; i < test_data->nentries; ++i) {
        ...
        for (int kv_bitmap = 0; kv_bitmap < 0x4; ++kv_bitmap) {
            ...
            const char *rest_keys[nents_in_hash];
...
TEST(hash, str_iterator_next_err_einval)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
    ...
    for (int n = 0; n < 2 * UPDATE_OP_END_MARK; ++n) {
        ...
        if (update_op == UPDATE_OP_PUT) {
            ...
            EXPECT_EQ(0, pfc_strhash_remove(hash, put_key));
        }
        ...
        if (call_next_before_update) {
            ...
            const char *key;
            pfc_cptr_t val;
            EXPECT_EQ(0, pfc_hashiter_next(it, (pfc_cptr_t *)&key, &val));
        }

        // update hash table before pfc_hashiter_next()
        switch (update_op) {
            case UPDATE_OP_PUT: {
                EXPECT_EQ(0, pfc_strhash_put(hash, put_key, put_val, 0));
                break;
            }
            case UPDATE_OP_UPDATE: {
                const char *update_key = test_data->key[0];
                pfc_cptr_t update_val = test_data->val_at_first[0];
                EXPECT_EQ(0, pfc_strhash_update(hash, update_key, update_val, 0));
                break;
            }
            case UPDATE_OP_REMOVE: {
                const char *rm_key = test_data->key[0];
        ...
        // pfc_hashiter_next() should return EINVAL after update
        {
            const char *key;
            pfc_cptr_t val;
            EXPECT_EQ(EINVAL, pfc_hashiter_next(it, (pfc_cptr_t *)&key, &val));
        }
...
TEST(hash, str_iterator_next_err_ebadf)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
    ...
    for (int n = 0; n < 2; ++n) {
...

        {
            ...
            const char *key;
            pfc_cptr_t val;
            EXPECT_EQ(EBADF, pfc_hashiter_next(it, (pfc_cptr_t *)&key, &val));
        }
    }
...
TEST(hash, str_iterator_remove)
{
    ...
    for (int n = 0; n < 3; ++n) {
        ...
        while (pfc_hashiter_next(it, (pfc_cptr_t *)&key_refptr, NULL) == 0) {
            char *key_str = strdup(pfc_refptr_string_value(key_refptr));
            EXPECT_EQ(0,      pfc_strhash_get(hash, key_str, NULL));
            EXPECT_EQ(0,      pfc_hashiter_remove(it));
            EXPECT_EQ(ENOENT, pfc_strhash_get(hash, key_str, NULL));
            EXPECT_EQ(ENOENT, pfc_hashiter_remove(it));
            free(key_str);
            ...
            for (int i = 0; i < nentries; ++i) {
                ...
                if (n == 0) {
                    snprintf(val[n][i], maxlen,
                             "value to put with key = %p = \"%s\"",
                             key[i], key[i]);
                } else {
                    snprintf(val[n][i], maxlen,
                             "value to update/%d with key = %p = \"%s\"",
                             n, key[i], key[i]);
                }
            }
        }
    ...
    for (int n = 0; n < val_nsets; ++n) {
        ...
        for (int i = 0; i < nentries; ++i) {
            if (n == 0 && empty_hash) {
                ...
                {
                    pfc_cptr_t get_val = NULL;
                    int get_err = pfc_strhash_get(hash, key[i], &get_val);
                    EXPECT_EQ(ENOENT, get_err);
                    EXPECT_TRUE(get_val == NULL);
                ...
                if (input_op == INPUT_OP_PUT) {
                    ...
                    int put_err = pfc_strhash_put(hash, key[i],
                                                  val[n][i], 0);
                    EXPECT_EQ(0, put_err);
                    ...
                    int update_err = pfc_strhash_update(hash, key[i],
                                                        val[n][i], 0);
                ...
                bool existing = (pfc_strhash_get(hash, key[i], NULL) == 0);

                // update for existing entry
                int update_err = pfc_strhash_update(hash, key[i], val[n][i], 0);
                EXPECT_EQ(0, update_err);
            ...
            {
                int get_err = pfc_strhash_get(hash, key[i], NULL);
                EXPECT_EQ(0, get_err);
            ...
            {
                int put_err = pfc_strhash_put(hash, key[i], val[n][i], 0);
        ...
        if ((remove_all_foreach != 0) && (n < val_nsets - 1)) {
            if (n % remove_all_foreach == 0) {
                ...
                for (int i = 0; i < nentries; ++i) {
                    ...
                    {
                        int remove_err = pfc_strhash_remove(hash, key[i]);
                        EXPECT_EQ(0, remove_err);
