...
/*
 * tests for hash using refptr key
 */
...
{
    // nbuckets pattern 1:
    //   each of [1, 3) and variations
    ssize_t max_nbuckets1 = ((!is_under_heavy_load()) ? 3 : 2);
...
TEST(hash, kref_create_zero_bucket)
{
    pfc_hash_t hash;
    int err = pfc_refhash_create(&hash, NULL, strint_ops, 0, 0);
    EXPECT_NE(0, err);
}


TEST(hash, kref_create_over_buckets)
{
    pfc_hash_t hash;
    int err = pfc_refhash_create(&hash, NULL, strint_ops,
                                 PFC_HASH_MAX_NBUCKETS + 1, 0);
    EXPECT_NE(0, err);
}


TEST(hash, kref_create_null_kops)
{
    ...
    {
        pfc_hash_t hash;
        EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, nbuckets, 0));
        pfc_hash_destroy(hash);
    }
...
TEST(hash, kref_create_user_kops)
{
    for (size_t nbuckets = 1;
         nbuckets <= 100;
         ++nbuckets)
    {
        pfc_hash_t hash;
        EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, strint_ops, nbuckets, 0));
        pfc_hash_destroy(hash);
    }
...
TEST(hash, kref_put_none_refptr_key)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));

    int i = 1234;
    int *key = &i;
    char val[] = "sample value";
    EXPECT_EQ(EINVAL, pfc_hash_put(hash, key, val, 0));

    pfc_hash_destroy(hash);
}


TEST(hash, kref_put_none_refptr_key_null)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));
...
TEST(hash, kref_put_mismatching_refptr_ops)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));

    pfc_refptr_t *key = strint_create_from_int(1234);
    EXPECT_EQ(1U, key->pr_refcnt);
    char val[] = "sample value";
    EXPECT_EQ(EINVAL, pfc_hash_put(hash, key, val, PFC_HASHOP_KEY_REFPTR));
    EXPECT_EQ(1U, key->pr_refcnt);
...
TEST(hash, kref_put_null_refptr_key)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));

    pfc_refptr_t *key = NULL;
    char val[] = "sample value";
    EXPECT_EQ(0, pfc_hash_put(hash, key, val, PFC_HASHOP_KEY_REFPTR));
    EXPECT_EQ(0, pfc_hash_remove(hash, key, PFC_HASHOP_KEY_REFPTR));

    pfc_hash_destroy(hash);
}


TEST(hash, kref_update_none_refptr_key)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));
    ...
    int *key = &i;
    char val[] = "sample value";
    EXPECT_EQ(EINVAL, pfc_hash_update(hash, key, val, 0));

    pfc_hash_destroy(hash);
}


TEST(hash, kref_update_none_refptr_key_null)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));
...
TEST(hash, kref_update_mismatching_refptr_ops)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));
    ...
    EXPECT_EQ(1U, key->pr_refcnt);
    char val[] = "sample value";
    EXPECT_EQ(EINVAL, pfc_hash_update(hash, key, val, PFC_HASHOP_KEY_REFPTR));
    EXPECT_EQ(1U, key->pr_refcnt);
...
TEST(hash, kref_update_null_refptr_key)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, NULL, NULL, 31, 0));
    ...
    pfc_refptr_t *key = NULL;
    char val[] = "sample value";
    EXPECT_EQ(0, pfc_hash_update(hash, key, val, PFC_HASHOP_KEY_REFPTR));
    EXPECT_EQ(0, pfc_hash_remove(hash, key, PFC_HASHOP_KEY_REFPTR));
...
TEST(hash, kref_put_update_key_null_ops)
{
    // init test data
    int nbuckets = 31;
    int nentries = nbuckets / 2;
...
TEST(hash, kref_get_remove_with_none_refptr_key_and_null_ops)
{
    test_get_remove(REFPTR_DEFAULT, /* type of key */
                    REFPTR_NONE,    /* type of value */
                    0);             /* get / remove with refptr key or not */
}


TEST(hash, kref_get_remove_with_refptr_key_and_null_ops)
{
    test_get_remove(REFPTR_DEFAULT, /* type of key */
                    REFPTR_NONE,    /* type of value */
                    1);             /* get / remove with refptr key or not */
}


TEST(hash, kref_clear_null_ops)
{
    test_clear(REFPTR_DEFAULT,  /* type of key */
               REFPTR_NONE);    /* type of value */
}


TEST(hash, kref_destroy_null_ops)
{
    test_destroy(REFPTR_DEFAULT,  /* type of key */
                 REFPTR_NONE);    /* type of value */
}


TEST(hash, kref_set_capacity_null_ops)
{
    test_set_capacity(REFPTR_DEFAULT,  /* type of key */
                      REFPTR_NONE);    /* type of value */
}


TEST(hash, kref_put_update_key_user_ops)
{
    // init test data
    int nbuckets = 31;
    int nentries = nbuckets / 2;
...
TEST(hash, kref_get_remove_with_none_refptr_key_and_user_ops)
{
    test_get_remove(REFPTR_STRINT,  /* type of key */
                    REFPTR_NONE,    /* type of value */
                    0);             /* get / remove with refptr key or not */
}


TEST(hash, kref_get_remove_with_refptr_key_and_user_ops)
{
    test_get_remove(REFPTR_STRINT,  /* type of key */
                    REFPTR_NONE,    /* type of value */
                    1);             /* get / remove with refptr key or not */
...
TEST(hash, kref_clear_user_ops)
{
    test_clear(REFPTR_STRINT,   /* type of key */
               REFPTR_NONE);    /* type of value */
}


TEST(hash, kref_destroy_user_ops)
{
    test_destroy(REFPTR_STRINT,   /* type of key */
                 REFPTR_NONE);    /* type of value */
}


TEST(hash, kref_set_capacity_user_ops)
{
    test_set_capacity(REFPTR_STRINT,   /* type of key */
                      REFPTR_NONE);    /* type of value */
}
