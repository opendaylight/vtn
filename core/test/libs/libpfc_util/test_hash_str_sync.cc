...
/*
 * tests for strhash synch block used by multi-threads
 */
...
#define TEST_HASH_SYNC(SYNC, NAME, ACTION)                          \
    TEST(hash, str_ ## SYNC ## _ ## NAME)                           \
    {                                                               \
        pfc_hash_t hash;                                            \
        EXPECT_EQ(0, pfc_strhash_create(&hash, NULL, nbuckets, 0)); \
        EXPECT_EQ(0, pfc_strhash_put(hash, test_key, test_val, 0)); \
...
TEST_HASH_WSYNC(get,
                pfc_cptr_t val;
                EXPECT_EQ(0, pfc_strhash_get(hash, test_key, &val));
...
TEST_HASH_WSYNC(put,
                EXPECT_EQ(0, pfc_strhash_put(hash, test_key2, &test_val2, 0));
                );

TEST_HASH_WSYNC(update,
                EXPECT_EQ(0, pfc_strhash_update(hash, test_key, &test_val, 0));
                );

TEST_HASH_WSYNC(remove,
                EXPECT_EQ(0, pfc_strhash_remove(hash, test_key));
                );
...
test_ww_sync_thr1 (void *arg)
{
    ...
    pfc_cptr_t val;
    EXPECT_EQ(0, pfc_strhash_get(hash, test_key2, &val));
...
test_ww_sync_thr2 (void *arg)
{
    ...
    pfc_hash_wsync_begin(hash);     // start wsync block
    EXPECT_EQ(0, pfc_strhash_update(hash, test_key2, test_val3, 0));
    pfc_hash_sync_end();            // end wsync block
    return NULL;
}

TEST(hash, str_wsync_and_wsync)
{
    // create hash
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_strhash_create(&hash, NULL, nbuckets, 0));
...
test_rw_sync_thr2 (void *arg)
{
    ...
    usleep(100 * 1000);             // sleep 100 msec
    pfc_hash_wsync_begin(hash);     // start wsync block
    EXPECT_EQ(0, pfc_strhash_remove(hash, test_key2));
    pfc_hash_sync_end();            // end rsync block
    return 0;
}

TEST(hash, str_rsync_and_wsync)
{
    // create hash
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_strhash_create(&hash, NULL, nbuckets, 0));
...
//     pfc_hash_rsync_begin(hash);     // start rsync block
//     EXPECT_EQ(ENOENT, pfc_strhash_get(hash, test_key2, NULL));
//     pfc_hash_sync_end();            // end rsync block
//     return 0;
// }
//
// TEST(hash, str_rsync_wsync_and_rsync)
// {
//     // create hash
//     pfc_hash_t hash;
//     EXPECT_EQ(0, pfc_strhash_create(&hash, NULL, nbuckets, 0));
