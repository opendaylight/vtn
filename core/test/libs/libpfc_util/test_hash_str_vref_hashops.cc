...
TEST(hash, str_vref_hashops_destroy_dtor_count)
{
    // init test data
    int nbuckets = 31;
    ...
    const pfc_hash_ops_t   *hash_ops = hash_wtchops_kptr_vref;
...
TEST(hash, str_vref_hashops_clear)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, str_vref_hashops_put_update)
{
    for (int k = 0; k < 4; ++k) {
...
TEST(hash, str_vref_hashops_put_equals_hashfunc_count)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, str_vref_hashops_update_equals_hashfunc_count)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, str_vref_hashops_put_update_null_key_val)
{
    // This test complements `TEST(hash, str_vref_hashops_put_update)' which
...
TEST(hash, str_vref_hashops_get)
{
    // init test data
    int nbuckets = 31;
    ...
    test_data_inject_null(test_data);
    TESTDATA_DECL_MEMBER_ACCESSOR(test_data);
    test_data_free(test_data);  // TODO: remove debug code
    return;                     // TODO: remove debug code

    // create hash
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_strhash_create(&hash, hash_ops, nbuckets, 0));
    pfc_hash_destroy(hash);     // TODO: remove debug code
    return;                     // TODO: remove debug code
...
TEST(hash, str_vref_hashops_remove)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 4; // keep more than '2' for injecting NULL cases
...
TEST(hash, str_vref_hashops_get_size_1)
{
    for (int nbuckets = 1;
         nbuckets < 300;
         ++nbuckets)
...
TEST(hash, str_vref_hashops_get_size_2)
{
...
TEST(hash, str_vref_hashops_get_set_capacity_1)
{
...
TEST(hash, str_vref_hashops_get_set_capacity_2)
{
...
TEST(hash, str_vref_hashops_get_set_capacity_3)
{
...
TEST(hash, str_vref_hashops_set_capacity_error)
{
    // create hash
    int nbuckets = 31;
...
TEST(hash, str_vref_hashops_report)
{
...
TEST(hash, str_vref_hashops_iterator_get_next_put)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, str_vref_hashops_iterator_next_err_einval)
{
    // init test data
    size_t nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, str_vref_hashops_iterator_next_err_ebadf)
{
    // init test data
    size_t nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, str_vref_hashops_iterator_remove)
{
    // init test data
    size_t nbuckets = 31;
    int nents = nbuckets * 2;
