...
TEST(hash, gen_create_zero_bucket)
{
    pfc_hash_t hash;
    int err = pfc_hash_create(&hash, NULL, 0, 0);
    EXPECT_NE(0, err);
}


TEST(hash, gen_create_over_buckets)
{
    pfc_hash_t hash;
    int err = pfc_hash_create(&hash, NULL, PFC_HASH_MAX_NBUCKETS + 1, 0);
    EXPECT_NE(0, err);
}


TEST(hash, gen_create_good_buckets_1)
{
...
TEST(hash, gen_create_good_buckets_2)
{
...
TEST(hash, gen_create_good_buckets_3)
{
    for (size_t nbuckets = 1;
         nbuckets <= PFC_HASH_MAX_NBUCKETS;
...
TEST(hash, gen_clear)
{
    ...
    pfc_hash_t hash;
    int err = pfc_hash_create(&hash, NULL, nbuckets, 0);
    EXPECT_EQ(0, err);
...
TEST(hash, gen_put_update2)
{
    // This test complements `TEST(hash, gen_put_update)' which
...
TEST(hash, gen_get_set_capacity_1)
{
    ssize_t max_nbuckets = ((!is_under_heavy_load()) ? 80 : 30);
...
TEST(hash, gen_get_set_capacity_2)
{
...
TEST(hash, gen_get_set_capacity_3)
{
...
TEST(hash, gen_set_capacity_error)
{
    // init hash
    int nbuckets = 31;
    pfc_hash_t hash;
...
TEST(hash, gen_iterator_get_next_put)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
...
TEST(hash, gen_iterator_next_err_einval)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
...
TEST(hash, gen_iterator_next_err_ebadf)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
