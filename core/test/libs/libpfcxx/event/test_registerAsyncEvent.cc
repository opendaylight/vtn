...
#define EVS "evs_cxx_regAsyncEvent"

TEST(registerAsyncEvent, name)
{
	char event_name[BUFSIZE];

	pfc_log_debug("registerAsyncEvent_name() is called");
...
#ifndef SKIP_ON_LRT
		ASSERT_EQ(EEXIST, Event::registerAsyncEvent(event_name));
#endif
...

	{
		...
		ASSERT_EQ(0, Event::registerAsyncEvent(event_name)); // Rewrite
		ASSERT_EQ(0, Event::unregisterEvent(event_name, &timeout_ev_cc_)); // Cleanup
	}
}
