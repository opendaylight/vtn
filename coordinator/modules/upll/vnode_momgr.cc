...
upll_rc_t VnodeMoMgr::RestoreVnode(ConfigKeyVal *ikey,
        ...
        if ((ikey)->get_cfg_val()) {
          ...
          case UNC_KT_VROUTER: { 
            val_vrt *vrt_val = reinterpret_cast<val_vrt *>
                                                 (GetVal(ikey));
            if (vrt_val != NULL) {
              vrt_val->valid[UPLL_IDX_DHCP_RELAY_ADMIN_STATUS_VRT] = UNC_VF_INVALID;
            }
          }
            break;
          case UNC_KT_VLINK: { 
    ...
    if (UPLL_DT_CANDIDATE == req->datatype ) {
      if (ikey->get_key_type() != UNC_KT_VLINK) {
        ...
        if (result_code != UPLL_RC_ERR_NO_SUCH_INSTANCE) {
          ...
          /* Renamed VTN existance check in Running DB */
          result_code = CheckRenamedVtnName(ikey, UPLL_DT_RUNNING, ctrlr_dom, dmi);
          if (result_code != UPLL_RC_ERR_NO_SUCH_INSTANCE) {
            if (UPLL_RC_SUCCESS == result_code) {
              UPLL_LOG_DEBUG("Specified VTN is already on controller");
              return UPLL_RC_ERR_GENERIC;
  ...
  if (key_type == UNC_KT_VBRIDGE || key_type == UNC_KT_VROUTER || key_type == UNC_KT_VTERMINAL) {
     ...
     */
    result_code = CheckVtnExistenceOnController(ikey, req, ctrlr_dom, restore_flag, dmi);
    if (result_code != UPLL_RC_SUCCESS) {
      UPLL_LOG_DEBUG("VTN doesn't exist on controller");
      return result_code;
    }
...
upll_rc_t VnodeMoMgr::CheckVtnExistenceOnController(
  ...
  if (UPLL_RC_ERR_NO_SUCH_INSTANCE == result_code) {
      ...
      if (auto_rename) {
        ...
        if ((UPLL_RC_SUCCESS == result_code) && !restore_flag) {
  ...
  while (tkey_next != NULL) {
    ...
    switch (ktype) {
      ...
      case UNC_KT_VTUNNEL:
        ...
        break;
      case UNC_KT_VTERMINAL:
        oper_status_change = SetOperStatus<val_vterm_st_t, val_db_vterm_st_t *>(
                                        tkey,  dmi, notification, true, save_to_db);
        break;
...
//      return false;
    ...
    case kAdminStatusEnabled:
      if ((vn_valst->down_count == 0) && (vn_valst->fault_count == 0)) {
        ...
        //oper_change = (vn_val->oper_status != UPLL_OPER_STATUS_DOWN)?
        oper_change = (vn_val->oper_status == UPLL_OPER_STATUS_UP)?
                     true:false;
        vn_val->oper_status = UPLL_OPER_STATUS_DOWN;
      }
      break;
      ...
      if (result_code == UPLL_RC_SUCCESS) {
        ...
        if (result_code == UPLL_RC_ERR_INSTANCE_EXISTS) {
          ...
          if (tkey->get_key_type() == UNC_KT_VBRIDGE) {
            ...
            result_code = vlan_mgr->UpdateConfigDB(ckv_vlanmap, UPLL_DT_STATE, UNC_OP_READ,
                                                   dmi, &dbop, MAINTBL);
            if (result_code == UPLL_RC_ERR_INSTANCE_EXISTS) {
