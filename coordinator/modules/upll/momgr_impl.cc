  ...
  if (!restore_flag) {
    if (UPLL_DT_CANDIDATE == req->datatype) {
      ...
      if (result_code == UPLL_RC_ERR_INSTANCE_EXISTS) {
        ...
        if (UPLL_RC_SUCCESS == result_code) {
          result_code = RestoreChildren(ikey, req->datatype, UPLL_DT_RUNNING, dmi, req);
...
upll_rc_t MoMgrImpl::TxVoteCtrlrStatus(unc_key_type_t keytype,
  ...
  // Retrieve the config Key as such from the KeyInfo
    for (err_key = cv_status_ptr->err_ckv; err_key;
      ...
      if (UNC_KT_VBR_IF != err_key->get_key_type()) {
        ...
        result_code = mgr->GetChildConfigKey(vtn_key, NULL);
        if (UPLL_RC_SUCCESS != result_code) {
           UPLL_LOG_DEBUG("GetChildConfigKey failed");
           return result_code;
        }
        ...
        key_vbr_if *if_key = reinterpret_cast<key_vbr_if_t *>(err_key->get_key());

        uuu::upll_strncpy(key->vtn_name,if_key->vbr_key.vtn_key.vtn_name,
                          kMaxLenVtnName+1);
        result_code = mgr->GetRenamedUncKey(vtn_key, UPLL_DT_CANDIDATE, dmi, ctrlr_id);
        if ((result_code != UPLL_RC_SUCCESS) &&
            (result_code != UPLL_RC_ERR_NO_SUCH_INSTANCE)) {
          UPLL_LOG_TRACE("Error in getting renamed name ");
          delete vtn_key;
      ...
      if (err_key->get_key_type() == UNC_KT_VBR_VLANMAP) {
        ...
        result_code = vlanmap_mgr->ReadConfigDB(vlanmap_ckv, UPLL_DT_CANDIDATE,
        ...
        ConfigKeyVal* err_next = err_key->get_next_cfg_key_val();
        err_key->set_next_cfg_key_val(NULL);
        err_key->ResetWith(err_ckv);
        err_key->AppendCfgKeyVal(err_next);
      ...
      if (err_key->get_key_type() == UNC_KT_VBR_IF) {
        /* Checks whether the received VBR_IF ConfigKeyVal error is
         * VTERM_IF Configuration or not. If it is VTERM_IF configuration
         * means translated into VTERM_IF error */
        ...
        result_code = vtermif_mgr->TranslateVbrIfToVtermIfError(trans_vtermiferr,
                                            err_key, UPLL_DT_CANDIDATE, dmi, ctrlr_id);
        /* Received error is VTERM_IF error */
        if (result_code == UPLL_RC_SUCCESS) {
          if (trans_vtermiferr) {
            err_key->ResetWithoutNextCkv(trans_vtermiferr);
          ...
          /* Received error is not VTERM_IF error */
          DELETE_IF_NOT_NULL(trans_vtermiferr);
          UPLL_LOG_DEBUG("Not a VTERM_IF error");
          ...
          vn_if_type iftype;
          SET_USER_DATA_CTRLR(err_key, ctrlr_id);
          ...
          if (result_code == UPLL_RC_SUCCESS) {
            if ((iftype == kVlinkBoundaryNode1) || (iftype == kVlinkBoundaryNode2)) {
              err_key->ResetWithoutNextCkv(ck_vlink);
...
upll_rc_t MoMgrImpl::TxUpdateController(unc_key_type_t keytype,
  ...
  while (result_code == UPLL_RC_SUCCESS) {
    ...
    /* If port map is configured on the requested interface 
     * value structure need to send driver */
    if ((ck_main->get_key_type() == UNC_KT_VTERM_IF) && (op == UNC_OP_DELETE)) {
      ...
      val_vterm_if_t *vtermif_val = reinterpret_cast<val_vterm_if_t *>
        (GetVal(ck_main));
      if (vtermif_val->valid[UPLL_IDX_PM_VTERMI] != UNC_VF_VALID) {
    ...
    if(IS_REDIRECT_INTERFACE_KEY(keytype) &&
       (op == UNC_OP_DELETE || op == UNC_OP_UPDATE)) {
      ...
      result_code = CheckVnodeInterfaceForRedirection(ck_main,
                                                    nreq,
                                                    dmi,
                                                    UPLL_DT_CANDIDATE,
                                                    op);
      ...
      if ((keytype == UNC_KT_VBR_FLOWFILTER_ENTRY)
          && (op != UNC_OP_DELETE) ) {
        // Perform the Semantic check for NWM which is reffered in the
        // flowfilter_entry. If the referred NWM is deleted then return
        // SEMANTIC ERROR
        result_code = PerformSemanticCheckForNWM(ck_main, dmi, dt_type); 
        if (result_code != UPLL_RC_SUCCESS) {
        ...
        result_code =  VerifyRedirectDestination(ck_main,
                                              dmi,
                                              UPLL_DT_CANDIDATE);
      if (result_code != UPLL_RC_SUCCESS) {
        UPLL_LOG_DEBUG(" VerifyRedirectDestination  result code - %d",result_code );
        *err_ckv = ck_main;
        DELETE_IF_NOT_NULL(req);
        DELETE_IF_NOT_NULL(nreq);
        dmi->CloseCursor(dal_cursor_handle, true);
      ...
      if ((op == UNC_OP_DELETE) &&
          (ck_main->get_key_type() == UNC_KT_VBR_NWMONITOR)) {
        // Verify whether given Network Monitor is referred in flowfilter
        // entry or not. If it is referred then return semantic error
        result_code  = ValidateNWM(ck_main, UPLL_DT_CANDIDATE, dmi);
        if (result_code != UPLL_RC_SUCCESS) {
      ...
      if (ccStatusPtr->upll_ctrlr_result != UPLL_RC_SUCCESS) {
        for (ConfigKeyVal *ck_err = ccStatusPtr->err_ckv; ck_err != NULL;
          ...
          if (!OVERLAY_KT(key_type)) {
            /* if renamed obtain the unc specifc name */
            if (UNC_KT_VBR_IF != ck_err->get_key_type()) {
              ...
              /* Vtn_key should not be null so avoid the null check */
              key_vtn *key = reinterpret_cast<key_vtn_t *>(vtn_key->get_key());
              key_vbr_if *if_key = reinterpret_cast<key_vbr_if_t *>(ck_err->get_key());
        ...
        /* VlanmapOnBoundary: Added vlanmap check */
          if (ck_err->get_key_type() == UNC_KT_VBR_VLANMAP) {
            ...
            ConfigKeyVal* err_next = ck_err->get_next_cfg_key_val();
            ck_err->set_next_cfg_key_val(NULL);
            ck_err->ResetWith(err_ckv);
            ck_err->AppendCfgKeyVal(err_next);
            DELETE_IF_NOT_NULL(err_ckv);
            continue;
          }
          ...
          if (ck_err->get_key_type() == UNC_KT_VBR_IF) {
            ...
            result_code = vtermif_mgr->TranslateVbrIfToVtermIfError(
                           trans_vtermiferr, ck_err, UPLL_DT_CANDIDATE, dmi, ctrlr_id);
            /* Received error is VTERM_IF error */
            if (result_code == UPLL_RC_SUCCESS) {
              if (trans_vtermiferr) {
                ck_err->ResetWithoutNextCkv(trans_vtermiferr);
                DELETE_IF_NOT_NULL(trans_vtermiferr);
              }
              continue;
            } else if (result_code != UPLL_RC_ERR_NO_SUCH_INSTANCE) {
              ...
              vn_if_type iftype;
              SET_USER_DATA_CTRLR(ck_err, ctrlr_id);
              ...
              if (result_code == UPLL_RC_SUCCESS) {
                if ((iftype == kVlinkBoundaryNode1) ||
                  ...
                  ck_err->ResetWithoutNextCkv(ck_vlink);
                  DELETE_IF_NOT_NULL(ck_vlink);
                  continue;
                } else {
                  if (ck_vlink) delete ck_vlink;
    ...
    for (int tbl = MAINTBL; tbl < ntable; tbl++) {
      ...
      if (tbl_index < uudst::kDalNumTables && tbl_index != 0) {
        ...
        dal_result = dmi->CopyModifiedRecords(UPLL_DT_CANDIDATE,
                      UPLL_DT_IMPORT, tbl_index, dal_bind_info, UNC_OP_CREATE);
...
upll_rc_t MoMgrImpl::AuditUpdateController(unc_key_type_t keytype,
  ...
  while (uud::kDalRcSuccess == (db_result = dmi->GetNextRecord(cursor))) {
    ...
    if (!OVERLAY_KT(keytype)) {
      ...
      /* During VTERM_IF delete request, if portmap is present means need to
       * send value structure to driver */
      if ((ckv_drvr->get_key_type() == UNC_KT_VTERM_IF) && (op == UNC_OP_DELETE)) {
       UPLL_LOG_TRACE("VTERM_IF  is  %s", ckv_drvr->ToStrAll().c_str());
        DbSubOp dbop = { kOpReadSingle, kOpMatchNone, kOpInOutNone};
        result_code = ReadConfigDB(ckv_drvr, UPLL_DT_AUDIT, UNC_OP_READ,
                      dbop, dmi, MAINTBL);
       UPLL_LOG_TRACE("VTERM_IF  is  %s", ckv_drvr->ToStrAll().c_str());
        if (result_code != UPLL_RC_SUCCESS) {
          UPLL_LOG_DEBUG("Returning error %d", result_code);
        ...
        val_vterm_if_t *vtermif_val = reinterpret_cast<val_vterm_if_t *>
          (GetVal(ckv_drvr));
        if (vtermif_val->valid[UPLL_IDX_PM_VTERMI] != UNC_VF_VALID) {
          ckv_drvr->DeleteCfgVal();
        }
      }

      if((keytype == UNC_KT_VBR_FLOWFILTER_ENTRY)
         && (op != UNC_OP_DELETE) ) {
        result_code = PerformRedirectTranslationForAudit(ckv_drvr,
                                                         dmi,
                                                         dt_type);
        if (UPLL_RC_SUCCESS != result_code) {
          UPLL_LOG_DEBUG("Unable to perform redirect node translation err %d",
                         result_code);
          DELETE_IF_NOT_NULL(ckv_running);
          DELETE_IF_NOT_NULL(ckv_audit);
          DELETE_IF_NOT_NULL(ckv_drvr);
    ...
    default:
      ...
      for (ckv_drv_rslt  = vote_status->err_ckv;
        ...
        if (!OVERLAY_KT(keytype)) {
          ...
          /* Checks whether the received VBR_IF ConfigKeyVal error is
           * VTERM_IF Configuration/boundary vlink /VBR_IF.
           * If it is VTERM_IF configuration means translated into VTERM_IF error
           * if it is vlink translate in to vlink.*/
          if (ckv_drv_rslt->get_key_type() == UNC_KT_VBR_IF) {
            ...
            ConfigKeyVal *trans_vtermiferr = NULL;
            result_code = vtermif_mgr->TranslateVbrIfToVtermIfError(
                trans_vtermiferr, ckv_drv_rslt, UPLL_DT_RUNNING, dmi, ctrlr_id);
            /* Received error is VTERM_IF error */
            if (result_code == UPLL_RC_SUCCESS) {
              if (trans_vtermiferr) {
              ...
              /* Translates VBR_IF error ConfigKeyVal to Vlink ConfigKeyVal error
               * if it is part of boundary vlink */
              ConfigKeyVal *ck_vlink = NULL;
              VlinkMoMgr *mgr = reinterpret_cast<VlinkMoMgr *>(
                  const_cast<MoManager *>(GetMoManager(UNC_KT_VLINK)));
              if (!mgr) {
              ...
              result_code = mgr->CheckIfMemberOfVlink(ckv_drv_rslt,
              ...
              if (result_code == UPLL_RC_SUCCESS) {
                if ((iftype == kVlinkBoundaryNode1) || (iftype == kVlinkBoundaryNode2)) {
                  ...
                  if (ck_vlink) delete ck_vlink;
                  UPLL_LOG_TRACE("%s is an internal vlink", (ckv_drv_rslt->ToStr()).c_str());
                }
              } else if (result_code != UPLL_RC_ERR_NO_SUCH_INSTANCE) {
                UPLL_LOG_DEBUG("Returning error %d", result_code);
                if (ck_vlink) delete ck_vlink;
                ...
                if (ck_vlink) delete ck_vlink;
                UPLL_LOG_TRACE("%s is not part of a vlink", (ckv_drv_rslt->ToStr()).c_str());
              }
            }
          }
          ...
          if (ckv_drv_rslt->get_key_type() == UNC_KT_VBR_VLANMAP) {
            ...
            ConfigKeyVal *vlanmap_ckv = NULL;
            VlanMapMoMgr *vlanmap_mgr = reinterpret_cast<VlanMapMoMgr *>(
                const_cast<MoManager *>(GetMoManager(ckv_drv_rslt->get_key_type())));
            ...
            result_code = vlanmap_mgr->ReadConfigDB(vlanmap_ckv, UPLL_DT_RUNNING,
                UNC_OP_READ, dbop, dmi, MAINTBL);
            if (result_code != UPLL_RC_SUCCESS) {
              UPLL_LOG_DEBUG("ReadConfigDB from running failed %d", result_code);
              DELETE_IF_NOT_NULL(vlanmap_ckv);
            ...
            ConfigKeyVal* drv_next_ckv = ckv_drv_rslt->get_next_cfg_key_val();
            ckv_drv_rslt->set_next_cfg_key_val(NULL);
            ckv_drv_rslt->ResetWith(err_ckv);
            ckv_drv_rslt->AppendCfgKeyVal(drv_next_ckv);
            DELETE_IF_NOT_NULL(err_ckv);
          }
          if (ckv_drv_rslt->get_key_type() != keytype)
            continue;
...
// AdaptErrValToVtn() will be called only during AUDIT case,
    ...
    switch(ckv_drv_rslt->get_key_type()) {
      ...
      case UNC_KT_VBR_IF: {
        ...
        if (IpctSt::kIpcStPfcdrvValVbrIf == cval->get_st_num()) {
          ...
          if (vbr_if_val->valid[PFCDRV_IDX_VEXTERNAL_NAME_VBRIF] == UNC_VF_VALID) {
...
upll_rc_t MoMgrImpl :: SetRedirectNodeAndPortForRead
    ...
    UPLL_FUNC_TRACE;
    upll_rc_t result_code = UPLL_RC_SUCCESS;
    VbrIfMoMgr *mgrvbrif = NULL;
    VtermIfMoMgr *mgrvtermif = NULL;
    VrtIfMoMgr *mgrvrtif = NULL;
    ConfigKeyVal * okey = NULL ;
    ...
    if ((val_entry->valid[UPLL_IDX_REDIRECT_NODE_FFE] ==
         UNC_VF_VALID) &&
          ...
          UNC_VF_VALID))  {
         result_code =  GetRenamedUncRedirectedNode(
                                      val_entry,
                                      dmi ,
                                      ctrlr_dom.ctrlr);
      if (result_code != UPLL_RC_SUCCESS) {
      ...
      // in response val_entry structure so UPLL set it as valid
      // redirect_direction as IN
      val_entry->valid[UPLL_IDX_REDIRECT_DIRECTION_FFE] = UNC_VF_VALID;
      val_entry->redirect_direction = UPLL_FLOWFILTER_DIR_IN;
      mgrvrtif = reinterpret_cast<VrtIfMoMgr *>
         (const_cast<MoManager *>(GetMoManager(UNC_KT_VRT_IF)));
      if (NULL == mgrvrtif) {
        UPLL_LOG_DEBUG("Unable to get VTERM Interface object");
        return UPLL_RC_ERR_GENERIC;
      }
      result_code = mgrvrtif->GetChildConfigKey(okey, NULL);
      if (result_code != UPLL_RC_SUCCESS) {
        UPLL_LOG_DEBUG("Memory allocation failed for VRT_IF key struct - %d",
            result_code);
        DELETE_IF_NOT_NULL(okey);
        return result_code;
      }
      ...
      key_vrt_if_t *vrtif_key = static_cast<key_vrt_if_t*>(
          okey->get_key());
      // SetVtnNmaeInRedirectNodeAndPortinRead will Set the Vtn_Name
      // in okey With respect to requested Kts in ikey
      result_code = SetVtnNameInRedirectNodeAndPortForRead(ikey , okey);
      if (result_code != UPLL_RC_SUCCESS) {
      ...
      uuu::upll_strncpy(vrtif_key->vrt_key.vrouter_name,
          ...
          (val_entry->redirect_node),
          (kMaxLenVnodeName + 1));
      uuu::upll_strncpy(vrtif_key->if_name,
          reinterpret_cast<char *>
          (val_entry->redirect_port),
          kMaxLenInterfaceName + 1);
      UPLL_LOG_DEBUG("ctrlrid %s, domainid %s",
          ctrlr_dom.ctrlr, ctrlr_dom.domain);
      SET_USER_DATA_CTRLR_DOMAIN(okey, ctrlr_dom);
      /* Check vtnnode and interface exists in table*/
      result_code = mgrvrtif->UpdateConfigDB(okey, UPLL_DT_RUNNING,
                                   UNC_OP_READ, dmi, &dbop_up, MAINTBL);
      if (UPLL_RC_ERR_INSTANCE_EXISTS == result_code) {
        UPLL_LOG_DEBUG("VRT_IF in val struct exists"
            "in DB");
        DELETE_IF_NOT_NULL(okey);
        return UPLL_RC_SUCCESS;
     } else if (UPLL_RC_ERR_NO_SUCH_INSTANCE == result_code) {
        UPLL_LOG_DEBUG("VRT_IF does not  exist in val struct exists"
            "in DB");
       DELETE_IF_NOT_NULL(okey);
       mgrvtermif = reinterpret_cast<VtermIfMoMgr *>
         (const_cast<MoManager *>(GetMoManager(UNC_KT_VTERM_IF)));
       if (NULL == mgrvtermif) {
         UPLL_LOG_DEBUG("Unable to get VTERM Interface object");
         return UPLL_RC_ERR_GENERIC;
       ...
       key_vterm_if_t *vterm_key = static_cast<key_vterm_if_t*>(
          okey->get_key());
       // SetVtnNmaeInRedirectNodeAndPortinRead will Set the Vtn_Name
       // in okey With respect to requested Kts in ikey
       result_code = SetVtnNameInRedirectNodeAndPortForRead(ikey , okey);
       ...
       uuu::upll_strncpy(vterm_key->vterm_key.vterminal_name,
          reinterpret_cast<char *>
          (val_entry->redirect_node),
          (kMaxLenVnodeName + 1));
       uuu::upll_strncpy(vterm_key->if_name,
          reinterpret_cast<char *>
          (val_entry->redirect_port),
          kMaxLenInterfaceName + 1);
       UPLL_LOG_DEBUG("ctrlrid %s, domainid %s",
       ...
       result_code = mgrvtermif->UpdateConfigDB(okey, UPLL_DT_RUNNING,
                                   UNC_OP_READ, dmi, &dbop_up, MAINTBL);
       if (UPLL_RC_ERR_INSTANCE_EXISTS == result_code) {
         UPLL_LOG_DEBUG("VTERM_IF in val struct exists"
            "in DB");
        ...
        DELETE_IF_NOT_NULL(okey);
        mgrvbrif = reinterpret_cast<VbrIfMoMgr *>
        (const_cast<MoManager *>(GetMoManager(UNC_KT_VBR_IF)));
        ...
        key_vbr_if_t *vbr_ifkey = static_cast<key_vbr_if_t*>(okey->get_key());
        // SetVtnNmaeInRedirectNodeAndPortinRead will Set the Vtn_Name
        // in tkey With respect to requested Kts in ikey
        result_code = SetVtnNameInRedirectNodeAndPortForRead(ikey , okey);
        if (result_code != UPLL_RC_SUCCESS) {
          UPLL_LOG_DEBUG("SetVtnNameInRedirectNodeAndPortForRead Fails - %d",
            result_code);
        ...
        uuu::upll_strncpy(vbr_ifkey->vbr_key.vbridge_name,
                      reinterpret_cast<char *>
                        (val_entry->redirect_node),
                           (kMaxLenVnodeName + 1));
        uuu::upll_strncpy(vbr_ifkey->if_name,
                   reinterpret_cast<char *>
                     (val_entry->redirect_port),
                           kMaxLenInterfaceName + 1);

        SET_USER_DATA_CTRLR_DOMAIN(okey, ctrlr_dom);
        result_code = mgrvbrif->UpdateConfigDB(okey, UPLL_DT_RUNNING,
                                UNC_OP_READ, dmi, &dbop_up, MAINTBL);

        if (result_code == UPLL_RC_ERR_INSTANCE_EXISTS) {
          UPLL_LOG_DEBUG("vbr/vbrif name of val_ff_entry "
           "exist in DB no need to translate it- %d",
            result_code);
          DELETE_IF_NOT_NULL(okey);
          return UPLL_RC_SUCCESS;
        } else if (result_code == UPLL_RC_ERR_NO_SUCH_INSTANCE) {
          ...
          okey->SetCfgVal(new ConfigVal(IpctSt::kIpcStPfcdrvValVbrIf, drv_vbr_if_val));
          // SetVtnNmaeInRedirectNodeAndPortinRead will Set
          // the Vtn_Name in okey with respect to to requested Kts
          result_code = SetVtnNameInRedirectNodeAndPortForRead(ikey , okey);
          if (result_code != UPLL_RC_SUCCESS) {
            UPLL_LOG_DEBUG("SetVtnNameInRedirectNodeAndPortForRead Fails - %d",
            result_code);
          ...
          UPLL_LOG_DEBUG("ctrlrid %s, domainid %s",
           ctrlr_dom.ctrlr, ctrlr_dom.domain);
          UPLL_LOG_DEBUG("vexte %s, vexternalif %s",
...
upll_rc_t MoMgrImpl :: SetVtnNameInRedirectNodeAndPortForRead
                                   (ConfigKeyVal *ck_main ,
                                     ConfigKeyVal *okey) {
  UPLL_FUNC_TRACE;
  if ((okey == NULL) || (ck_main == NULL)) {
    UPLL_LOG_DEBUG("Requested okey is NULL ");
    return UPLL_RC_ERR_GENERIC;
  }
  key_vbr_if_t *vbrif_key = NULL;
  key_vterm_if_t *vterm_key = NULL;
  key_vrt_if_t*  vrtif_key  = NULL ;
  if (okey->get_key_type() == UNC_KT_VBR_IF) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBR_IF");
    vbrif_key = static_cast<key_vbr_if_t*>(
        okey->get_key());
  } else if (okey->get_key_type() == UNC_KT_VTERM_IF) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERM_IF");
    vterm_key = static_cast<key_vterm_if_t*>(
          okey->get_key());
  } else if (okey->get_key_type() == UNC_KT_VRT_IF) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VRT_IF");
    vrtif_key = static_cast<key_vrt_if_t*>(
          okey->get_key());
  }
  if (ck_main->get_key_type() == UNC_KT_VBRIF_FLOWFILTER_ENTRY) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER_ENTRY");
    key_vbr_if_flowfilter_entry_t *key_vbr_if_ffe =
      reinterpret_cast<key_vbr_if_flowfilter_entry_t *>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER_ENTRY"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
           key_vbr_if_ffe->flowfilter_key.if_key.vbr_key.vtn_key.vtn_name,
            kMaxLenVtnName + 1);
    } else  if (okey->get_key_type() == UNC_KT_VTERM_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER_ENTRY"
           "Okey is  UNC_KT_VTERM_IF");
      uuu::upll_strncpy(vterm_key->vterm_key.vtn_key.vtn_name,
              key_vbr_if_ffe->flowfilter_key.if_key.vbr_key.vtn_key.vtn_name,
                kMaxLenVtnName + 1);
    } else  if (okey->get_key_type() == UNC_KT_VRT_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER_ENTRY"
  ...
  } else if (ck_main->get_key_type() == UNC_KT_VRTIF_FLOWFILTER_ENTRY) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VRTIF_FLOWFILTER_ENTRY");
    key_vrt_if_flowfilter_entry_t *key_vrt_if_ffe =
      reinterpret_cast<key_vrt_if_flowfilter_entry_t *>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VRTIF_FLOWFILTER_ENTRY"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
      ...
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER_ENTRY"
           "Okey is  UNC_KT_VRT_IF");
      uuu::upll_strncpy(vrtif_key->vrt_key.vtn_key.vtn_name,
              key_vrt_if_ffe->flowfilter_key.if_key.vrt_key.vtn_key.vtn_name,
                kMaxLenVtnName + 1);
    }
  } else if (ck_main->get_key_type() == UNC_KT_VBR_FLOWFILTER_ENTRY) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBR_FLOWFILTER_ENTRY");
    key_vbr_flowfilter_entry_t *key_vbr_ffe =
    reinterpret_cast<key_vbr_flowfilter_entry_t*>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBR_FLOWFILTER_ENTRY"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
  ...
  } else if (ck_main->get_key_type() == UNC_KT_VBR_FLOWFILTER) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBR_FLOWFILTER");
    key_vbr_flowfilter_t *key_vbr_ff =
      reinterpret_cast<key_vbr_flowfilter_t*>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBR_FLOWFILTER"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
  ...
  } else if (ck_main->get_key_type() == UNC_KT_VBRIF_FLOWFILTER) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER");
    key_vbr_if_flowfilter_t *key_vbr_if_ff =
      reinterpret_cast<key_vbr_if_flowfilter_t*>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
        key_vbr_if_ff->if_key.vbr_key.vtn_key.vtn_name,
            kMaxLenVtnName + 1);
    } else  if (okey->get_key_type() == UNC_KT_VTERM_IF) {
        UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VBRIF_FLOWFILTER"
        "Okey is  UNC_KT_VTERM_IF");
  ...
  } else if (ck_main->get_key_type() == UNC_KT_VRTIF_FLOWFILTER) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VRTIF_FLOWFILTER");
    key_vrt_if_flowfilter_t *key_vrt_if_ff =
     reinterpret_cast<key_vrt_if_flowfilter_t*>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
        key_vrt_if_ff->if_key.vrt_key.vtn_key.vtn_name,
            kMaxLenVtnName + 1);
    } else if (okey->get_key_type() == UNC_KT_VTERM_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER"
        "Okey is  UNC_KT_VTERM_IF");
  ...
  } else if (ck_main->get_key_type() == UNC_KT_VTERMIF_FLOWFILTER) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER");
    key_vterm_if_flowfilter_t *key_vterm_if_ff =
     reinterpret_cast<key_vterm_if_flowfilter_t*>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER"
        "Okey is  UNC_KT_VBR_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
        key_vterm_if_ff->if_key.vterm_key.vtn_key.vtn_name,
            kMaxLenVtnName + 1);
    } else if (okey->get_key_type() == UNC_KT_VTERM_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER"
        "Okey is  UNC_KT_VTERM_IF");
  ...
  } else if (ck_main->get_key_type() == UNC_KT_VTERMIF_FLOWFILTER_ENTRY) {
    UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER_ENTRY");
    key_vterm_if_flowfilter_entry_t *key_vterm_if_ffe =
      reinterpret_cast<key_vterm_if_flowfilter_entry_t *>(ck_main->get_key());
    if (okey->get_key_type() == UNC_KT_VBR_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VTERMIF_FLOWFILTER_ENTRY"
        "Okey is  UNC_KT_VTERM_IF");
      uuu::upll_strncpy(vbrif_key->vbr_key.vtn_key.vtn_name,
           key_vterm_if_ffe->flowfilter_key.if_key.vterm_key.vtn_key.vtn_name,
            kMaxLenVtnName + 1);
    } else  if (okey->get_key_type() == UNC_KT_VTERM_IF) {
      UPLL_LOG_DEBUG("Requested Keytype is UNC_KT_VRTIF_FLOWFILTER_ENTRY"
        "Okey is  UNC_KT_VTERM_IF");
