...
#ifndef UNC_UPLL_UNIFIED_NW_LABEL_MOMGR_H
#define UNC_UPLL_UNIFIED_NW_LABEL_MOMGR_H
...
class UNWLabelMoMgr : public MoMgrImpl {
  private:
    static BindInfo unw_label_bind_info[];
    static unc_key_type_t unw_label_child[];
    /**
    ...
    upll_rc_t GetValid(void *val,
      ...
      if (tbl == MAINTBL) {
        switch (indx) {
          case uudst::unw_label::kDbiMaxCount:
            valid = &(reinterpret_cast<val_unw_label *>(val))->
                valid[UPLL_IDX_MAX_COUNT_UNWL];
            break;
          case uudst::unw_label::kDbiRaisingThreshold:
            valid = &(reinterpret_cast<val_unw_label *>(val))->
                valid[UPLL_IDX_RAISING_THRESHOLD_UNWL];
            break;
          case uudst::unw_label::kDbiFallingThreshold:
            valid = &(reinterpret_cast<val_unw_label *>(val))->
                valid[UPLL_IDX_FALLING_THRESHOLD_UNWL];
            break;
          default:
  ...
  upll_rc_t ValidateUnwLabelKey(key_unw_label_t *unw_label_key,
                                unc_keytype_operation_t operation);

  upll_rc_t ValidateUnwLabelVal(IpcReqRespHeader * req,
                                    ConfigKeyVal * ikey);
