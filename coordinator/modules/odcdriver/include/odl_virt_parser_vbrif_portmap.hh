#ifndef __ODL_VIRT_PARSER_VBRIF_PORTMAP_HH__
#define __ODL_VIRT_PARSER_VBRIF_PORTMAP_HH__
...
class VirtParserVbrifPortmapCmd {
private:
...
odc_drv_resp_code_t convert_logical_port(
 ...
 UncRespCode set_vlan(json_object *out, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) { 
  int vlanid(0);
  if ((UNC_VF_VALID == val.val_vbrif.portmap.valid[UPLL_IDX_VLAN_ID_PM])
      &&
      (UNC_VF_VALID == val.val_vbrif.portmap.valid[UPLL_IDX_TAGGED_PM])) {
    if (val.val_vbrif.portmap.tagged == UPLL_VLAN_TAGGED) {
      vlanid = val.val_vbrif.portmap.vlan_id;
    }
  }
  if (vlanid >= 0) {
 ...
 UncRespCode set_type(json_object *out, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) {
 ...
 UncRespCode set_id(json_object *out, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) {
 ...
 UncRespCode set_name(json_object *out, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) {
 ...
 UncRespCode get_vlan(json_object *in, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) { 
   val.val_vbrif.valid[UPLL_IDX_PM_VBRI] = UNC_VF_VALID;
   int vlanid(0);
   ...
   if (restjson::REST_OP_SUCCESS != ret_val) {
      ...
      if (0 == vlanid) {
        val.val_vbrif.portmap.valid[UPLL_IDX_VLAN_ID_PM] =
                                                   UNC_VF_INVALID;
        pfc_log_debug("untagged");
        val.val_vbrif.portmap.tagged = UPLL_VLAN_UNTAGGED;
        val.val_vbrif.portmap.valid[UPLL_IDX_TAGGED_PM] = UNC_VF_VALID;
      } else {
        pfc_log_debug("vlan id valid");
        val.val_vbrif.portmap.valid[UPLL_IDX_VLAN_ID_PM] = UNC_VF_VALID;
        val.val_vbrif.portmap.vlan_id = vlanid;
        pfc_log_debug("%d  vlan id ", vlanid);
        pfc_log_debug("vlan id tagged");
        val.val_vbrif.portmap.tagged = UPLL_VLAN_TAGGED;
        val.val_vbrif.portmap.valid[UPLL_IDX_TAGGED_PM] = UNC_VF_VALID;
      }
   return UNC_RC_SUCCESS;
 }

 UncRespCode get_type(json_object *in, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) {
   return UNC_RC_SUCCESS;
 }
 
 UncRespCode get_name(json_object *in, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) {
   std::string port_name = "";
   ...
   strncpy(reinterpret_cast<char*>
              (val.val_vbrif.portmap.logical_port_id),
              logical_port.c_str(),
              sizeof(val.val_vbrif.portmap.logical_port_id) - 1);
   pfc_log_debug("%s logical port id in readportmap " ,
                    reinterpret_cast<char*>
                    (val.val_vbrif.portmap.logical_port_id));
   val.val_vbrif.portmap.valid[UPLL_IDX_LOGICAL_PORT_ID_PM] = UNC_VF_VALID;
   return UNC_RC_SUCCESS;
 }

 UncRespCode get_id(json_object *in, key_vbr_if_t &key, pfcdrv_val_vbr_if_t &val) {
   std::string node_id = "";
...
std::string VirtParserVbrifPortmapCmd::logical_port = "";
}
}
#endif
