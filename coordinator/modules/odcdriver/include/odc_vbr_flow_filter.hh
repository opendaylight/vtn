...
public OdcFlowFilterCmd<key_vbr_flowfilter,
  val_flowfilter>
...
public:
  OdcVbrFlowFilterCmd (unc::restjson::ConfFileValues_t conf_values):
    OdcFlowFilterCmd<key_vbr_flowfilter,val_flowfilter>(conf_values),
    parent_vtn_name_(""),
    parent_vbr_name_(""),
    is_out(PFC_FALSE) {}
  UncRespCode
  ...
  void copy(flowfilter *out, key_vbr_flowfilter &key_in,
  ...
  UncRespCode r_copy(flowfilterlist* in,
    ...
    strncpy(reinterpret_cast<char*> (key_filter.vbr_key.vbridge_name),
            parent_vbr_name_.c_str(), sizeof(key_filter.vbr_key.vbridge_name) - 1);
    strncpy(reinterpret_cast<char*> (key_filter.vbr_key.vtn_key.vtn_name),
            parent_vtn_name_.c_str(), sizeof(key_filter.vbr_key.vtn_key.vtn_name) - 1);
    if (is_out == PFC_FALSE)
    ...
    unc::vtndrvcache::ConfigNode *filter_cfgptr =
      new unc::vtndrvcache::CacheElementUtil<key_vbr_flowfilter, val_flowfilter, uint32_t>
    ...
    while ( entry_iter != in->flowfilter_.end() ) {
      ...
      unc::vtndrvcache::ConfigNode *entry_cfgptr=
        new unc::vtndrvcache::CacheElementUtil<key_vbr_flowfilter_entry, val_flowfilter_entry, uint32_t>
...
public OdcFlowFilterCmd<key_vbr_flowfilter_entry,
...
public:
  OdcVbrFlowFilterEntryCmd(unc::restjson::ConfFileValues_t conf_values):
    OdcFlowFilterCmd<key_vbr_flowfilter_entry,val_flowfilter_entry>(conf_values),
  ...
  void copy(flowfilter *out, key_vbr_flowfilter_entry &key_in,
    ...
    pfc_log_info("check DSCP Valid %d",  value_in.valid[UPLL_IDX_PRIORITY_FFE]);

    if ( value_in.valid[UPLL_IDX_DSCP_FFE] == UNC_VF_VALID ) {
      pfc_log_info("New DSCP Action Createed");
    ...
    if ( value_in.valid[UPLL_IDX_PRIORITY_FFE] == UNC_VF_VALID ) {
      pfc_log_info("New Priority Action Createed");
      action *new_action = new action();
