#ifndef _ODC_VIRTUAL_VBR_HH_
#define _ODC_VIRTUAL_VBR_HH_

namespace unc {
namespace odcdriver{

class VirtualVbrCommand {
public:
  VirtualVbrCommand() {}
  ~VirtualVbrCommand() {}
 UncRespCode set_vbridge_ageinterval(json_object *in, key_vbr_t& key, val_vbr_t& val) {
   int age_interval_ = 600;
   //if ((age_interval_ >=10) && (age_interval_<= 1000000)) {
    // pfc_log_info(" valid age_interval_");
 ...
 UncRespCode get_controller_name( unc::driver::controller* ctr, key_vbr_t& key, val_vbr_t& val) {
 ...
 UncRespCode get_domain_name( unc::driver::controller* ctr, key_vbr_t& key, val_vbr_t& val) {
