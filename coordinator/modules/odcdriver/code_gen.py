...
def put_method(item, file_desc, members, output_file_name, d):
    print 'put'
    url_name = parser.ReadValues(item, members)['url']
    http_respcode = parser.ReadValues(item, members)['http_success']
    implementation_put = 'static  UncRespCode %s (%s &intf,json_object* in) {' %(members, url_name) +'\n'
    implementation_put = implementation_put + '\t' + 'pfc_log_info("Entering in to get");'+'\n'
    implementation_put = implementation_put + '\t' + 'std::string url(intf.get_url());' + '\n'
    implementation_put = implementation_put + '\t' + 'unc::restjson::RestUtil rest_util_obj(intf.get_controller_ip(),intf.get_user_name(),intf.get_password());' + '\n'
    implementation_put = implementation_put + '\t' + 'if(unc::restjson::RestUtil rest_util_obj(HTTP_METHOD_PUT, NULL, url,  ctr->controller_ip_,ctr->controller_port_,ctr->audit_) == %s)'%(http_respcode) + '{' + '\n'
    implementation_put = implementation_put + '\t\t' + 'return UNC_RC_SUCCESS;' +'\n'
    implementation_put = implementation_put + '\t' + '}' +'\n' + '\t' +  'else' + '\n'
    implementation_put = implementation_put + '\t' + 'return UNC_DRV_RC_ERR_GENERIC;' +'\n'
    d['implementation_put'] = implementation_put + '}' + '\n'
    f = open(output_file_name, "a")
    f.write(d['implementation_put'])
    f.close()

def get_nested_object(item, file_desc, members, output_file_name, d):
    print 'objects'
    print 'members', members
    url_name = parser.ReadValues(item, members)['url']
    req_members = parser.ReadValues(item, members)['request_members']
    member = parser.ReadValues(item, req_members)['members']
    http_status = parser.ReadValues(item, members)['http_success']
    objects = 'static UncRespCode get%s(%s &intf) {' %(members, url_name) +'\n'
    objects = objects + ' std::string url(intf.get_url());' + '\n'
    objects = objects + 'int ret_val = 0;' +'\n'
    objects = objects + 'unc::restjson::RestUtil rest_util_obj(intf.get_controller_ip(),intf.get_user_name(),intf.get_password());' + '\n'
    objects = objects + 'if(rest_util_obj.send_http_response(GET, out, url,  ctr->controller_ip_,ctr->controller_port_,ctr->audit_) == %s){' %(http_status)+'\n'
    objects = objects + 'return UNC-RC_SUCCESS'+'\n'
    objects = objects + 'else' + '\n' + 'return UNC_DRV_RC_ERR_GENERIC'+'\n'+'}'+'\n'
    objects = objects + 'int ret_val = 1;'+ '\n'
    objects = objects + 'uint32_t array_length = 0;'+'\n'
    objects = objects + 'json_object *in = NULL;'+'\n'
    objects = objects + 'uint32_t ret_val = unc::restjson::JsonBuildParse::parse(in,%s);'%(members) + '\n'
    objects = objects + 'if(restjson::REST_OP_SUCCESS != ret_val)'+'\n'
    objects = objects + '{'+'return UNC_DRV_RC_ERR_GENERIC;'+'\n'+'}'+'\n'
    objects = objects + 'if (json_object_is_type(%in, json_type_array)) {' +'\n'
    objects = objects + ' array_length = unc::restjson::JsonBuildParse::get_array_length(in, %s);'%(req_members)+'\n'+'}'+'\n'
    objects = objects + ' for (uint32_t arr_idx = 0; arr_idx < array_length; arr_idx++)'+'\n'+'{'
    objects = objects + 'json_object *json_obj_interface_idx = NULL;'+'\n'
    objects = objects + 'get_array_idx(&json_obj_interface_idx, in, arr_idx);'+'\n'
    for sub_members in member.split(','):
        objects = objects + 'ret_val = get%s(%s,in)'%(sub_members, req_members) + '\n'
        objects = objects + 'if(UNC_RC_SUCCESS != ret_val)' + '\n'
        objects = objects + 'return UNC_DRV_RC_ERR_GENERIC;' + '\n'
    objects = objects + 'if(intf.read_callback(%s)!= UNC_RC_SUCCESS) {'%(req_members) + '\n'
    objects = objects + 'return UNC_DRV_RC_ERR_GENERIC;'+ '\n'
    objects = objects + 'return UNC_RC_SUCCESS' + '\n'+ '}' +'\n'
    objects = objects + 'else' + '\n' + '{' +'\n' + 'return UNC_DRV_RC_ERR_GENERIC' +'\n' + '}' +'\n'+'}'+'\n'
    d['objects'] = objects + 'return UNC_RC_SUCCESS'+ '\n' + '}' +'\n'
    ...
    element_type = parser.ReadValues(item, sub_members)['type']
    if element_type == 'object':
        get_object(item, file_desc, members, output_file_name, d)
    if element_type == 'string':
        get_string(item, file_desc, members, output_file_name, d)
...
def get_array_idx():
    ...
    d['get_array_idx'] = get_array_idx + 'pfc_log_debug("arr_idx_one:%s", unc::restjson::JsonBuildParse::get_json_string(*out));' '\n' + '}'+'\n'
    file = open(output_file_name, "a")
    file = file.write(d['get_array_idx'])

def deletes_method(item, file_desc, members, output_file_name, d):
    print 'deletes'
    url_name = parser.ReadValues(item, members)['url']
    print url_name
    http_respcode = parser.ReadValues(item, members)['http_success']
    implementation_delete = 'static UncRespCode %s (%s &intf) {' %(members, url_name.lower()) +'\n'
    implementation_delete = implementation_delete + '\t' + 'std::string url(intf.get_url());' + '\n'
    implementation_delete = implementation_delete +  '\t' + 'unc::restjson::RestUtil rest_util_obj(intf.get_controller_ip(),intf.get_user_name(),intf.get_password());' + '\n'
    implementation_delete = implementation_delete + '\t' + 'if(unc::restjson::RestUtil rest_util_obj(DELETE, url,  ctr->controller_ip_,ctr->controller_port_,ctr->audit_) == %s)'%(http_respcode) + '{' + '\n'
    implementation_delete = implementation_delete + '\t\t' + 'return UNC_RC_SUCCESS;' +'\n'
    implementation_delete = implementation_delete + '\t' + '}' +'\n' + '\t' +  'else' + '\n'
    implementation_delete = implementation_delete + '\t' + 'return UNC_DRV_RC_ERR_GENERIC;' +'\n'
    implementation_delete = implementation_delete + 'return UNC_RC_SUCCESS'+'\n'
    d['implementation_delete'] = implementation_delete + '}' + '\n'
    f = open(output_file_name, "a")
    f.write(d['implementation_delete'])
    f.close()

def  int_method(item, file_desc, member, output_file_name, d):
    print 'rangeint'
    min = parser.ReadValues(item, member)['rangeint']
    max = parser.ReadValues(item, member)['rangemax']
    int_method = int_method + '\t\t' + 'if (range_check(in.%s,%s,%s)== UNC_RC_SUCCESS)' + '\n'
...
def validates_method(item, file_desc, members, output_file_name, d):
    print 'validates'
    request_members = parser.ReadValues(item, members)['request_members']
    print request_members
    for member in request_members.split(','):
        print member
        check_validate = parser.ReadValues(item, member).has_key('validation')
        print 'check_validation', check_validate
        validate = '\t' + 'Static UncRespCode %s(%s &in) {'%(members, request_members)  + '\n'
        if str(check_validate) == 'True':
            validation_value = parser.ReadValues(item, member)['validation']
            write_validation[validation_value](item, file_desc, member, output_file_name, d)
        else:
            validate = validate + '\t\t' +  'return UNC_RC_SUCCESS' + '\n'
            validate = validate + '}' + '\n'
            test['struct']['validate'] = validate
            file = open(output_file_name, "a")
            file.write(test['struct']['validate'])

write_validation = {'rangeint': int_method, 'fixed_string':fixed_string_method}

write_implementation_methods = {'POST': set_method, 'PUT': put_method, 'DELETE': deletes_method, 'VALIDATE':validates_method, 'GET':get_all_method, 'READ':read_all_method}
