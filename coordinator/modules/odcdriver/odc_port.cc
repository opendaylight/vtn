...
drv_resp_code_t OdcPort::parse_port_properties_value(int arr_idx,
  ...
  json_object *json_obj_node_prop_name;
  ...
  json_object *json_obj_node_prop_state;
  ...
  json_object *json_obj_node_prop_config;
  ...
  json_object *json_obj_node_prop_speed;
...
void OdcPort::fill_logical_port_val_structure(key_port_t *key_port,
  ...
  val_logical_port.logical_port.valid[VAL_PORT_EVENT_ATTR3] = UNC_VF_VALID;

  std::string port_id = reinterpret_cast<const char*> (key_port->port_id);
  strncpy(reinterpret_cast<char*>
          (val_logical_port.logical_port.physical_port_id),
          port_id.c_str(), strlen(port_id.c_str()));
  val_logical_port.logical_port.valid[VAL_PORT_EVENT_ATTR4]
      = UNC_VF_VALID;

  val_logical_port.logical_port.oper_down_criteria =
      UNC_OPERSTATUS_CRITERIA_ANY;
  val_logical_port.logical_port.valid[VAL_PORT_EVENT_ATTR5]
      = UNC_VF_VALID;
