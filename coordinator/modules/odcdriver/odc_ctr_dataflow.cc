...
UncRespCode OdcCtrDataFlowCommand::read_cmd(
  ...
  for (; it_vtn_name != vtn_names.end(); ++it_vtn_name) {
    ...
    std::string url = get_dataflow_url(*it_vtn_name, key_ctr_dataflow);
    if (url == "") {
    ...
    if (NULL != response->write_data) {
      if (NULL != response->write_data->memory) {
        ...
        UncRespCode ret_val =  parse_flow_response(key_ctr_dataflow,
                                                   df_util,
                                                   data);
...
std::string OdcCtrDataFlowCommand::get_dataflow_url(std::string vtn_name,
                                                 key_ctr_dataflow key) {
  ODC_FUNC_TRACE;
  std::string url = "";
  url.append(BASE_URL);
  ...
  // flow id
  unsigned int flow_id = key.flow_id;
  std::stringstream ss_flowid;
  ss_flowid << flow_id;
  std::string flow_str = ss_flowid.str();
  pfc_log_debug("%s: CTR dataflow flow id received:%d", PFC_FUNCNAME, flow_id);
  url.append(flow_str);
  pfc_log_debug("%s: final URL:%s", PFC_FUNCNAME, url.c_str());
  return url;
}

UncRespCode OdcCtrDataFlowCommand::parse_flow_response(
    key_ctr_dataflow_t& key_ctr_dataflow,
    unc::vtnreadutil::driver_read_util* df_util,
    char *data) {
  ODC_FUNC_TRACE;
  ...
  resp_code = parse_flow_response_values(key_ctr_dataflow,
                                         df_util,
                                         jobj,
                                         -1);
...
UncRespCode OdcCtrDataFlowCommand::parse_flow_response_values(
  ...
  if (option1 == UNC_OPT1_DETAIL) {
    json_object  *jobj_stat = NULL;
    ret_val = restjson::JsonBuildParse::parse(jobj_data, "statistics",
                                              arr_idx,jobj_stat);

    if (restjson::REST_OP_SUCCESS != ret_val) {
      pfc_log_error(" Error occured while statistics ");
      return UNC_DRV_RC_ERR_GENERIC;
    }

    ret_val = restjson::JsonBuildParse::parse(jobj_stat, "bytes",
                                              -1, bytes);
    if (restjson::REST_OP_SUCCESS != ret_val) {
      pfc_log_error(" Error occured while parsing bytes");
      return UNC_DRV_RC_ERR_GENERIC;
    }

    ret_val = restjson::JsonBuildParse::parse(jobj_stat, "duration",
                                              -1, duration);
    if (restjson::REST_OP_SUCCESS != ret_val) {
      pfc_log_error(" Error occured while parsing duration");
      return UNC_DRV_RC_ERR_GENERIC;
    }

    ret_val = restjson::JsonBuildParse::parse(jobj_stat, "packets",
                                              -1, packets);
    if (restjson::REST_OP_SUCCESS != ret_val) {
      pfc_log_error(" Error occured while parsing packets");
      return UNC_DRV_RC_ERR_GENERIC;
    }
