...
namespace driver {
// Constructor
ODLVBRIfCommand::ODLVBRIfCommand() {
...
const char* ODLVBRIfCommand::create_request_body_port_map(
...
uint32_t ODLVBRIfCommand::create_cmd_port_map(key_vbr_if_t& vbrif_key,
...
std::string ODLVBRIfCommand::get_vbrif_url(key_vbr_if_t& vbrif_key) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  char* vtnname = NULL;
...
uint32_t ODLVBRIfCommand::update_cmd_port_map(key_vbr_if_t& vbrif_key,
    pfcdrv_val_vbr_if_t& vbrif_val, unc::driver::controller *ctr_ptr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  PFC_ASSERT(ctr_ptr != NULL);
  ODCController *odc_ctr = reinterpret_cast<ODCController*> (ctr_ptr);
...
uint32_t ODLVBRIfCommand::delete_cmd_port_map(key_vbr_if_t& vbrif_key,
    pfcdrv_val_vbr_if_t& vbrif_val, unc::driver::controller *ctr_ptr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  PFC_ASSERT(ctr_ptr != NULL);
  ODCController *odc_ctr = reinterpret_cast<ODCController*> (ctr_ptr);
...
drv_resp_code_t ODLVBRIfCommand::create_cmd(key_vbr_if_t& vbrif_key,
  ...
  if (val.val_vbrif.valid[2] == UNC_VF_VALID) {
    retval = create_cmd_port_map(vbrif_key, val, odc_ctr);
...
drv_resp_code_t ODLVBRIfCommand::update_cmd(key_vbr_if_t& vbrif_key,
    pfcdrv_val_vbr_if_t& val, unc::driver::controller *ctr_ptr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  PFC_ASSERT(ctr_ptr != NULL);
  ODCController *odc_ctr = reinterpret_cast<ODCController*> (ctr_ptr);
  ...
  if ((val.val_vbrif.valid[2] == UNC_VF_VALID)
    ...
    retval = update_cmd_port_map(vbrif_key, val, odc_ctr);
    if (retval != ODC_DRV_SUCCESS) {
      return DRVAPI_RESPONSE_FAILURE;
    }
  } else if ((val.val_vbrif.valid[2] == UNC_VF_VALID_NO_VALUE)
      && (val.valid[0] == UNC_VF_VALID)) {
    retval = delete_cmd_port_map(vbrif_key, val, odc_ctr);
    if (retval != ODC_DRV_SUCCESS) {
      return DRVAPI_RESPONSE_FAILURE;
    }
  }
...
drv_resp_code_t ODLVBRIfCommand::delete_cmd(key_vbr_if_t& vbrif_key,
    pfcdrv_val_vbr_if_t& val, unc::driver::controller *ctr_ptr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  PFC_ASSERT(ctr_ptr != NULL);
  ODCController *odc_ctr = reinterpret_cast<ODCController*> (ctr_ptr);
...
const char* ODLVBRIfCommand::create_request_body(pfcdrv_val_vbr_if_t& val_vtn) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  restjson::JsonBuildParse json_obj;
...
drv_resp_code_t ODLVBRIfCommand::validate_op(key_vbr_if_t& key_vbr_if,
...
drv_resp_code_t ODLVBRIfCommand::validate_create_vbrif(key_vbr_if_t& key_vbr_if,
...
drv_resp_code_t ODLVBRIfCommand::validate_delete_vbrif(key_vbr_if_t& key_vbr_if,
    unc::driver::controller* ctr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  uint32_t resp_code = ODC_DRV_FAILURE;
  resp_code = is_vbrif_exists_in_controller(key_vbr_if, ctr);
...
drv_resp_code_t ODLVBRIfCommand::validate_update_vbrIf(key_vbr_if_t& key_vbr_if,
    unc::driver::controller* ctr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  uint32_t resp_code = ODC_DRV_FAILURE;
  resp_code = is_vbrif_exists_in_controller(key_vbr_if, ctr);
...
// Chacke if vtn exist in contrller or not
uint32_t ODLVBRIfCommand::is_vtn_exists_in_controller(key_vbr_if_t& key_vbr_if,
    unc::driver::controller* ctr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  char* vtnname = NULL;
...
uint32_t ODLVBRIfCommand::is_vbrif_exists_in_controller(
...
uint32_t ODLVBRIfCommand::is_vbr_exists_in_controller(key_vbr_if_t& key_vbr_if,
    unc::driver::controller* ctr) {
  pfc_log_debug("%s Entering function", PFC_FUNCNAME);
  char* vtnname = NULL;
  char* vbrname = NULL;
  ...
  url.append("/vbridges/");
  url.append(vbrname);
  uint32_t response_code = get_controller_response(url, odc_ctr);
  pfc_log_debug("Response code from Controller is : %d ", response_code);
  return response_code;
}

// Gets the controller respnse
uint32_t ODLVBRIfCommand::get_controller_response(std::string url,
