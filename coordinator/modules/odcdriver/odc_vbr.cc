...
drv_resp_code_t ODCVBRCommand::parse_vbrif_append_vector(string vtn_name,
                                     string vbr_name, json_object *json_obj,
                    int arr_idx, std::string url, unc::driver::controller* ctr,
  ...
  string description = "";
  string entity_state = "";
  int ret_val = unc::restjson::JsonBuildParse::parse(json_obj,
                                                     "name", arr_idx, name);
  if (ret_val) {
  ...
  ret_val = unc::restjson::JsonBuildParse::parse(json_obj,
                                  "description", arr_idx, description);
  if (ret_val) {
    return DRVAPI_RESPONSE_FAILURE;
  }
...
drv_resp_code_t ODCVBRCommand::get_vbr_child(string vtn_name,
                                             string vbr_name,
                                unc::driver::controller* ctr,
      vector<unc::vtndrvcache::ConfigNode *> &cfgnode_vector) {
  pfc_log_debug("Enter get_vbr_child");
  ...
  parse_ret = parse_vbrif_resp_data(vtn_name, vbr_name, url, ctr, data,
                                    cfgnode_vector);
  pfc_log_debug("Exit get_vbr_child");
  return parse_ret;
}
drv_resp_code_t ODCVBRCommand::parse_vbrif_resp_data(string vtn_name,
                                                     string vbr_name,
                            string url, unc::driver::controller* ctr,
                                                          char *data,
              vector<unc::vtndrvcache::ConfigNode *> &cfgnode_vector) {
  pfc_log_debug("Enter parse_vbrif_resp_data");
  ...
  int ret_val = unc::restjson::JsonBuildParse::parse(jobj, "interface",
                                                     -1, json_obj_vbrif);
  if (ret_val) {
    pfc_log_error("JsonBuildParse::parse fail");
    return DRVAPI_RESPONSE_FAILURE;
  ...
  pfc_log_debug("exit parse_vbrif_resp_data");
  return DRVAPI_RESPONSE_SUCCESS;
}
