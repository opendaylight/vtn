...
drv_resp_code_t OdcVbrVlanMapCommand::validate_vlan_exist(
  ...
  if (key_vlan_map.logical_port_id_valid != 0) {
    ...
    pfc_log_debug("%s: PortID %s", PFC_FUNCNAME, port_id.c_str());
...
OdcVbrVlanMapCommand::check_ANY_already_exists(key_vlan_map_t &key_vlan_map,
  ...
  for (std::vector<std::string>::iterator it = vtn_vbr_vlan_vector.begin();
    ...
    pfc_log_debug("%s: Contents of vector %s",
                  PFC_FUNCNAME, vtn_vbr_vlan_data.c_str());
  }
  ...
  for (std::vector<std::string>::iterator it = vtn_vbr_vlan_vector.begin();
    ...
    std::size_t dot_occurence = vtn_vbr_vlan_data.find(PERIOD);
    uint32_t flag = 0;
    while (dot_occurence != std::string::npos) {
      std::string parse_data  = vtn_vbr_vlan_data.substr(0, dot_occurence);
      ...
      vtn_vbr_vlan_data = vtn_vbr_vlan_data.substr(dot_occurence+1);
      dot_occurence = vtn_vbr_vlan_data.find(PERIOD);
      flag++;
    }
    ...
    if (vlan_id_req != vlan_id_ctr) {
      if (vtn_name_req.compare(vtn_name_ctr) == 0) {
        if (vbr_name_req.compare(vbr_name_ctr) == 0) {
          if (switch_id_ctr.compare(NODE_TYPE_ANY) == 0) {
...
drv_resp_code_t OdcVbrVlanMapCommand::check_switch_already_exists(
  ...
  for (std::vector<std::string>::iterator it = vtn_vbr_vlan_vector.begin();
       it != vtn_vbr_vlan_vector.end(); it++) {
    std::string vtn_vbr_vlan_data = *it;
    pfc_log_debug("%s: VLANID from vector (%s)", vtn_vbr_vlan_data.c_str(),
                  PFC_FUNCNAME);
  }
  for (std::vector<std::string>::iterator it = vtn_vbr_vlan_vector.begin();
       it != vtn_vbr_vlan_vector.end(); it++) {
    ...
    while (dot_occurence != std::string::npos) {
      ...
      if (flag == 0) {
        ...
        switch_id_ctr = parse_data;
        pfc_log_debug("%s: Switch id(%s)", PFC_FUNCNAME, switch_id_ctr.c_str());
      }
      vtn_vbr_vlan_data = vtn_vbr_vlan_data.substr(dot_occurence+1);
      dot_occurence = vtn_vbr_vlan_data.find(PERIOD);
      flag++;
    ...
    pfc_log_debug("vlan id req %d is same as vlanid in vector %d ",
                  vlan_id_req, vlan_id_ctr);
    pfc_log_debug("vtn,vbr req %s %s,and  vtn vbr in vector %s %s ",
                  vtn_name_req.c_str(), vbr_name_req.c_str(),
                  vtn_name_ctr.c_str(), vbr_name_ctr.c_str());
    pfc_log_debug("logical id req %s , logical id in vector %s",
                  logical_port_req.c_str(), switch_id_ctr.c_str());

    if (vlan_id_req == vlan_id_ctr) {
      if (vtn_name_req.compare(vtn_name_ctr) == 0) {
        if (vbr_name_req.compare(vbr_name_ctr) == 0) {
          if (switch_id_ctr == NODE_TYPE_ANY) {
            pfc_log_error("%s:vlan same , same vtn , vbr, ctr -> ANY",
                          PFC_FUNCNAME);
            return DRVAPI_RESPONSE_FAILURE;
      ...
      if (vtn_name_req.compare(vtn_name_ctr) == 0) {
        if (vbr_name_req.compare(vbr_name_ctr) == 0) {
          if (logical_port_req.compare(switch_id_ctr) == 0) {
            pfc_log_debug
                ("vlan diff , same vtn same vbr, req - SW ctr diff SW");
            is_switch_exist = PFC_TRUE;
            ...
            val_id_str_format << vlan_id_ctr;
            port_id.append(val_id_str_format.str());
            pfc_log_debug("%s port id in validate " , port_id.c_str());
          }
        }
      }
    }
...
// Getting  vbridge child if available(vlan map)
...
drv_resp_code_t OdcVbrVlanMapCommand::fill_config_node_vector(void *parent_key,
  ...
  vtn_vbr_vlan.append(parent_vtn_name);
  vtn_vbr_vlan.append(PERIOD);
  vtn_vbr_vlan.append(parent_vbr_name);
  vtn_vbr_vlan.append(PERIOD);
  vtn_vbr_vlan.append(id);
  unc::odcdriver::OdcController *odc_ctr =
      reinterpret_cast<unc::odcdriver::OdcController *>(ctr);
  PFC_ASSERT(odc_ctr != NULL);
  std::vector<std::string> vtn_vbr_vlan_vector = odc_ctr->vlan_vector;
  ...
  for (std::vector<std::string>::iterator it = vtn_vbr_vlan_vector.begin();
    ...
    if (vtn_vbr_vlan_data.compare(vtn_vbr_vlan) == 0) {
      pfc_log_debug("Already exist");
      is_data_exist = PFC_TRUE;
    }
  }
  if (PFC_FALSE == is_data_exist) {
    pfc_log_debug("vector is updated with %s", vtn_vbr_vlan.c_str());
...
std::string OdcVbrVlanMapCommand::generate_vlanmap_id(
  ...
  pfc_log_error("%s: Generated MapId:[%s] ", PFC_FUNCNAME, str_mapid.c_str());
  return str_mapid;
}
...
// It Delete vlan-map from controller
drv_resp_code_t OdcVbrVlanMapCommand::del_existing_vlanmap(
  ...
  pfc_log_debug("%s VBR VlanMap url to be deleted %s", PFC_FUNCNAME,
                del_vbr_vlanmap_url.c_str());
  if (del_vbr_vlanmap_url.empty()) {
    pfc_log_error("%s: vbr vlan url is empty", PFC_FUNCNAME);
    return DRVAPI_RESPONSE_FAILURE;
  }
  ...
  std::string vtn_vbr_vlan_delete = "";
  vtn_vbr_vlan_delete.append(vtn_name_req);
  vtn_vbr_vlan_delete.append(PERIOD);
  vtn_vbr_vlan_delete.append(vbr_name_req);
  vtn_vbr_vlan_delete.append(PERIOD);
  vtn_vbr_vlan_delete.append(str_mapping_id);
...
drv_resp_code_t OdcVbrVlanMapCommand::create_update_cmd(
  ...
  int resp_code = response->code;
  pfc_log_debug("Response code from Ctl for vlanmap create_cmd: %d", resp_code);
  rest_client_obj.clear_http_response();
  ...
  std::string vtn_vbr_vlan_update = "";
  vtn_vbr_vlan_update.append(vtn_name_req);
  vtn_vbr_vlan_update.append(PERIOD);
  vtn_vbr_vlan_update.append(vbr_name_req);
  vtn_vbr_vlan_update.append(PERIOD);
  uint vlanid =  vlanmap_val.vlan_id;
  pfc_log_debug("--vlanmap_val.vlan_id %d", vlanid);
  ...
  vtn_vbr_vlan_update.append(map_id);
  ...
  for (uint iterator = 0; iterator < odc_ctr->vlan_vector.size(); iterator++) {
    pfc_log_debug("before update values in vector in create update %d is %s",
                  iterator, odc_ctr->vlan_vector.at(iterator).c_str());
  }
  update_vector(ctr_ptr,  vtn_vbr_vlan_update);
  for (uint iterator = 0; iterator < odc_ctr->vlan_vector.size(); iterator++) {
    pfc_log_debug("after update values in vector in create update %d is %s",
                  iterator, odc_ctr->vlan_vector.at(iterator).c_str());
  }
  return DRVAPI_RESPONSE_SUCCESS;
}
...
void OdcVbrVlanMapCommand::delete_from_vector(unc::driver::controller *ctr ,
  ...
  unc::odcdriver::OdcController *odc_ctr =
      reinterpret_cast<unc::odcdriver::OdcController *>(ctr);
  PFC_ASSERT(odc_ctr != NULL);
  pfc_log_debug("to be delete--%s", vtn_vbr_vlan.c_str());
  for (uint iterator = 0; iterator < odc_ctr->vlan_vector.size(); iterator++) {
    pfc_log_debug("value in vector - %s",
                  odc_ctr->vlan_vector.at(iterator).c_str());
    if (odc_ctr->vlan_vector.at(iterator).compare(vtn_vbr_vlan) == 0) {
      odc_ctr->vlan_vector.erase(odc_ctr->vlan_vector.begin()+iterator);
      pfc_log_debug("One entry in list deleted %s", vtn_vbr_vlan.c_str());
    }
  }
}
...
OdcVbrVlanMapCommand::generate_vlanid_from_vector(unc::driver::controller *ctr,
  ...
  pfc_log_debug("req vtn vbr vlan is %s", vtn_vbr_vlan_req.c_str());
...
drv_resp_code_t OdcVbrVlanMapCommand::delete_cmd(key_vlan_map_t&
  ...
  pfc_log_debug("%s: In parent ptr - VTN [%s] VBR [%s] ", PFC_FUNCNAME,
                parent_vbr.vtn_key.vtn_name, parent_vbr.vbridge_name);
  std::string str_vlan_id = generate_vlanid_from_vector(ctr_ptr, vlanmap_key);
  if (str_vlan_id.empty()) {
    pfc_log_error("Empty vlan id returned ");
    return DRVAPI_RESPONSE_FAILURE;
  }
  std::string logical_portid_req = reinterpret_cast<char*>
      (vlanmap_key.logical_port_id);
  ...
  pfc_log_debug("mapping id formed in delete %s" , str_mapping_id.c_str());

  if (str_mapping_id.length() == 0) {
    pfc_log_error("%s: Vlan MapId generation failed :%s", PFC_FUNCNAME,
  ...
  int resp_code = response->code;
  pfc_log_debug("Response code from Ctl for delete vlanmap : %d ", resp_code);
  rest_client_obj.clear_http_response();
  if (HTTP_200_RESP_OK != resp_code) {
    pfc_log_error("delete vlanmap is not successful %d", resp_code);
    return DRVAPI_RESPONSE_FAILURE;
  }
  std::string vtn_name_req =
      reinterpret_cast<char*>(vlanmap_key.vbr_key.vtn_key.vtn_name);
  ...
  vtn_vbr_vlan_delete.append(PERIOD);
  uint vlanid =  vlanmap_val.vlan_id;
  pfc_log_debug("---------DELETE---------vlanmap_val.vlan_id %d", vlanid);
  std::ostringstream convert_vlanid;
  convert_vlanid << vlanid;

  vtn_vbr_vlan_delete.append(str_mapping_id);

  pfc_log_debug("---------DELETE---------map id %s", str_mapping_id.c_str());
  unc::odcdriver::OdcController *odc_ctr =
      reinterpret_cast<unc::odcdriver::OdcController *>(ctr_ptr);
  PFC_ASSERT(odc_ctr != NULL);

  for (uint iterator = 0; iterator < odc_ctr->vlan_vector.size(); iterator++) {
    pfc_log_debug("before update vallues in vector in delete %d is %s",
                  iterator, odc_ctr->vlan_vector.at(iterator).c_str());
  }
  delete_from_vector(ctr_ptr, vtn_vbr_vlan_delete);

  for (uint iterator = 0; iterator < odc_ctr->vlan_vector.size(); iterator++) {
    pfc_log_debug("after update vallues in vector in delete %d is %s",
                  iterator, odc_ctr->vlan_vector.at(iterator).c_str());
  }
...
json_object* OdcVbrVlanMapCommand::create_request_body(
  ...
  if (vlanmap_key.logical_port_id_valid != 0) {
    ...
    std::string logical_port_id = reinterpret_cast<char*>
        (vlanmap_key.logical_port_id);
    odc_drv_resp_code_t ret_value = validate_logical_port_id(logical_port_id);
    if (ret_value != ODC_DRV_SUCCESS) {
      pfc_log_error("%s: Validation of logical_port_id failed",
                    PFC_FUNCNAME);
      return NULL;
    }
    std::string switch_id = "";
