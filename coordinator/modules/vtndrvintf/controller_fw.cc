...
namespace driver {
    ...
    * a map  and also checks  that ping is needed for the particular
    * controller or not
    ...
    */
    ...
    VtnDrvRetEnum ReadParams::PingController() {
      ...
      if (ping_status == PFC_TRUE) {
        ...
        pfc_log_info("Controller staus is UP ctlr_name: %s",
                     ctlr_name_.c_str());
      } else {
        ctr_instance->set_connection_status(CONNECTION_DOWN);
        pfc_log_info("Controller staus is DOWN ctlr_name:%s",
                     ctlr_name_.c_str());
      }
    ...
    * @brief     : This function gets the driver instance and controller instance
    * for the appropriate without acquiring the lock
    * controllers
    * @param[in] : controller_name, controller*, driver*
    * @retval    : driver*
    */
