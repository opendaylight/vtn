...
/*
 * tests for hash that's created with PFC_HASH_NOLOCK flag
 */
...
TEST(hash, opt_nolock_create_zero_bucket)
{
    pfc_hash_t hash;
    int err = pfc_hash_create(&hash, NULL, 0, PFC_HASH_NOLOCK);
    EXPECT_NE(0, err);
}


TEST(hash, opt_nolock_create_over_buckets)
{
    pfc_hash_t hash;
    int err = pfc_hash_create(&hash, NULL, PFC_HASH_MAX_NBUCKETS + 1,
                              PFC_HASH_NOLOCK);
    EXPECT_NE(0, err);
}


TEST(hash, opt_nolock_create_good_buckets_1)
{
    ...
    {
        pfc_hash_t hash;
        EXPECT_EQ(0, pfc_hash_create(&hash, NULL, nbuckets, PFC_HASH_NOLOCK));
...
TEST(hash, opt_nolock_create_good_buckets_2)
{
...
TEST(hash, opt_nolock_create_good_buckets_3)
{
    for (size_t nbuckets = 1;
         nbuckets <= PFC_HASH_MAX_NBUCKETS;
...
TEST(hash, opt_nolock_clear)
{
    ...
    pfc_hash_t hash;
    int err = pfc_hash_create(&hash, NULL, nbuckets, PFC_HASH_NOLOCK);
    EXPECT_EQ(0, err);
...
TEST(hash, opt_nolock_put_update)
{
    // init test data
    int nbuckets = 31;
...
TEST(hash, opt_nolock_put_update2)
{
    // This test complements `TEST(hash, opt_nolock_put_update)' which
...
TEST(hash, opt_nolock_get_uint_val)
{
#ifdef PFC_LP64
...
TEST(hash, opt_nolock_get_size_1)
{
    for (int nbuckets = 1;
         nbuckets < 500;
         ++nbuckets)
...
TEST(hash, opt_nolock_get_size_2)
{
...
TEST(hash, opt_nolock_get_set_capacity_1)
{
...
TEST(hash, opt_nolock_get_set_capacity_2)
{
...
TEST(hash, opt_nolock_get_set_capacity_3)
{
...
TEST(hash, opt_nolock_set_capacity_error)
{
    // init hash
    int nbuckets = 31;
    pfc_hash_t hash;
...
TEST(hash, opt_nolock_iterator_get_next_put)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
...
TEST(hash, opt_nolock_iterator_next_err_einval)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
...
TEST(hash, opt_nolock_iterator_next_err_ebadf)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
...
TEST(hash, opt_nolock_iterator_remove)
{
    // init test data
    size_t nbuckets = 31;
    int nentries = nbuckets * 2;
