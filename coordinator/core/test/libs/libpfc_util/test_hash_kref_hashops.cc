...
TEST(hash, kref_hashops_create_null_kops)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_refhash_create(&hash, hash_wtchops_kref_vptr,
                                    NULL, 31, 0));
...
TEST(hash, kref_hashops_create_user_kops)
{
    pfc_refptr_ops_t empty_kops;
    memset(&empty_kops, 0, sizeof(empty_kops));
...
TEST(hash, kref_hashops_destroy_dtor_count)
{
    // init test data
    int nbuckets = 31;
    ...
    const pfc_hash_ops_t   *hash_ops = hash_wtchops_kref_vptr;
    ...
    for (int i = 0; i < nents; ++i) {
        ...
        // check with refptr key
        {
            ...
            EXPECT_EQ(vals[0][i], (pfc_refptr_t *)v);
        }
...
TEST(hash, kref_hashops_put_update)
{
    for (int k = 0; k < 4; ++k) {
...
TEST(hash, kref_hashops_put_equals_hashfunc_count)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_update_equals_hashfunc_count)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_put_update_null_key_val)
{
    // This test complements `TEST(hash, kref_hashops_put_update)' which
...
TEST(hash, kref_hashops_put_dummy_null_key)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_update_dummy_null_key)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_get)
{
    ...
    for (int i = 0; i < nents; ++i) {
        // do get with refptr key and non-NULL valuep
        {
            ...
            pfc_cptr_t v = INVALID_PTR;
            EXPECT_EQ(0, pfc_hash_get(hash, k, &v, kvref_flags));
            EXPECT_EQ(vals_at_tail[i], (pfc_refptr_t *)v);
...
TEST(hash, kref_hashops_remove)
{
    ...
    for (int i = 0; i < nents; ++i) {
        ...
        pfc_cptr_t expected_v = (pfc_cptr_t)vals_at_tail[i];
...
TEST(hash, kref_hashops_get_size_1)
{
    for (int nbuckets = 1;
         nbuckets < 500;
         ++nbuckets)
...
TEST(hash, kref_hashops_get_size_2)
{
...
TEST(hash, kref_hashops_get_set_capacity_1)
{
    ssize_t max_nbuckets = ((!is_under_heavy_load()) ? 50 : 10);
...
TEST(hash, kref_hashops_get_set_capacity_2)
{
...
TEST(hash, kref_hashops_get_set_capacity_3)
{
...
TEST(hash, kref_hashops_set_capacity_error)
{
    // create hash
    int nbuckets = 31;
    const pfc_refptr_ops_t *kref_ops = strint_ops;
...
TEST(hash, kref_hashops_iterator_get_next_put)
{
    // init test data
    int nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_iterator_next_err_einval)
{
    // init test data
    size_t nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_iterator_next_err_ebadf)
{
    // init test data
    size_t nbuckets = 31;
    int nents = nbuckets * 2;
...
TEST(hash, kref_hashops_iterator_remove)
{
    // init test data
    size_t nbuckets = 31;
    int nents = nbuckets * 2;
        ...
        for (int i = 0; i < nents; ++i) {
            ...
            // get for existing entry
            {
                // do get with refptr key
                {
                    ...
                    pfc_cptr_t v = INVALID_PTR;
                    EXPECT_EQ(0, pfc_hash_get(hash, k, &v, kvref_flags));
                    EXPECT_EQ(vals[n][i], (pfc_refptr_t *)v);
                }
