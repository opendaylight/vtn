...
/*
 * tests for u64hash using refptr value
 */
...
test_put_ents (test_data_t *test_data, test_input_op_t input_op)
{
    ...
    for (int i = 0; i < nentries; ++i) {
        for (int n = 0; n < val_nsets; ++n) {
            ...
            switch (input_op) {
                case INPUT_OP_PUT:
                    err = pfc_u64hash_put(hash, key, val, opflags);
                    break;
                case INPUT_OP_UPDATE:
                    err = pfc_u64hash_update(hash, key, val, opflags);
                    break;
                default:
                    abort();
            }
...
test_get_remove (test_refptr_mode_t val_refptr_mode)
{
    ...
    for (int n = 0; n < val_nsets; ++n) {
        // set values (and failour tests of pfc_u64hash_get / pfc_hash__remove)
        for (int i = 0; i < nentries; ++i) {
            // failure tests before set
            EXPECT_EQ(ENOENT, pfc_u64hash_get(hash, key[i], NULL));
            EXPECT_EQ(ENOENT, pfc_u64hash_remove(hash, key[i]));

            // set
            EXPECT_EQ(0, pfc_u64hash_put(hash, key[i], val[n][i], v_opflag));
        ...
        for (int i = 0; i < nentries; ++i) {
            ...
            // do get
            pfc_cptr_t v = NULL;
            EXPECT_EQ(0, pfc_u64hash_get(hash, key[i], &v));
            EXPECT_EQ(val[n][i], v);
        ...
        for (int i = 0; i < nentries; ++i) {
            ...
            // do remove
            EXPECT_EQ(0, pfc_u64hash_remove(hash, key[i]));
    ...
    for (int n = 0; n < val_nsets; ++n) {
        ...
        // test clear operation
        {
            ...
            if (n < (val_nsets - 1)) {
                ...
                for (int i = 0; i < nentries; ++i) {
                    EXPECT_EQ(ENOENT,
                              pfc_u64hash_get(hash, key[i], NULL));
                }
            }
            ...
            // check emptiness by iterator
            {
                ...
                while (pfc_hashiter_uint64_next(it, NULL, NULL) == 0) {
                    ++iter_cnt;
                }
                       ...
                       ssize_t *new_caps)
{
    ...
    for (ssize_t *next_cap = new_caps; *next_cap > 0; ++next_cap) {
        for (int n = 0; n < val_nsets; ++n) {
            ...
            for (int i = 0; i < nentries; ++i) {
                EXPECT_EQ(0, pfc_u64hash_get(hash, key[i], NULL));
            }
...
TEST(hash, u64_vref_put_null)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_u64hash_create(&hash, NULL, 31, 0));
    ...
    // put
    EXPECT_EQ(0, pfc_u64hash_put(hash, key, val, PFC_HASHOP_VAL_REFPTR));

    // check with get / remove operation
    {
        // get
        pfc_cptr_t v =(pfc_cptr_t)-1;
        EXPECT_EQ(0, pfc_u64hash_get(hash, key, &v));
        EXPECT_EQ(val, v);

        // remove
        EXPECT_EQ(0, pfc_u64hash_remove(hash, key));
    }
...
TEST(hash, u64_vref_update_null)
{
    pfc_hash_t hash;
    EXPECT_EQ(0, pfc_u64hash_create(&hash, NULL, 31, 0));
    ...
    EXPECT_EQ(0, pfc_u64hash_update(hash, key, val1, PFC_HASHOP_VAL_REFPTR));
    ...
    EXPECT_EQ(0, pfc_u64hash_update(hash, key, val2, PFC_HASHOP_VAL_REFPTR));
    ...
    EXPECT_EQ(0, pfc_u64hash_update(hash, key, val3, PFC_HASHOP_VAL_REFPTR));
...
TEST(hash, u64_vref_put_update_null_ops)
{
    // init test data
    int nbuckets = 31;
    int nentries = nbuckets / 3;
    int val_nsets = 2;
    test_data_t *test_data = test_data_alloc(nbuckets,
                                             nentries,
                                             val_nsets,
...
TEST(hash, u64_vref_get_remove_null_ops)
{
    test_get_remove(REFPTR_DEFAULT); /* type of value */
}


TEST(hash, u64_vref_clear_null_ops)
{
    test_clear(REFPTR_DEFAULT); /* type of value */
}


TEST(hash, u64_vref_destroy_null_ops)
{
    test_destroy(REFPTR_DEFAULT); /* type of value */
}


TEST(hash, u64_vref_set_capacity_null_ops)
{
    test_set_capacity(REFPTR_DEFAULT); /* type of value */
}


TEST(hash, u64_vref_put_update_user_ops)
{
    // init test data
    int nbuckets = 31;
    int nentries = nbuckets / 3;
...
TEST(hash, u64_vref_get_remove_user_ops)
{
    test_get_remove(REFPTR_STRINT);  /* type of value */
}


TEST(hash, u64_vref_clear_user_ops)
{
    test_clear(REFPTR_STRINT);  /* type of value */
}


TEST(hash, u64_vref_destroy_user_ops)
{
    test_destroy(REFPTR_STRINT);  /* type of value */
}


TEST(hash, u64_vref_set_capacity_user_ops)
{
...
{
    PFC_ASSERT(test_data != NULL);
    EXPECT_EQ(0, pfc_u64hash_create(hashp,
                                    NULL,
                                    test_data->nbuckets,
                                    0));
}
