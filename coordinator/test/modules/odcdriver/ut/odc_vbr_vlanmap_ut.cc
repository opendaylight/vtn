...
TEST(odcdriver,  test_fetch_conf_vtn_name_empty) {
  ...
  drv_resp_code_t ret_val = odc_vbr_vlanmap.fetch_config(ctr, &key_vbr, cfgnode_vector);
...
TEST(odcdriver,  test_fetch_conf_vlanmap_resp) {
  ...
  for ( std::vector<unc::vtndrvcache::ConfigNode *>::iterator it =
    ...
    if (*it !=  NULL) {
      ...
      if (cfg_node != NULL) {
        unc::vtndrvcache::CacheElementUtil<key_vlan_map_t, val_vlan_map_t, uint32_t> *cache_util_ptr =
            static_cast<unc::vtndrvcache::CacheElementUtil<key_vlan_map_t, val_vlan_map_t, uint32_t> *>(cfg_node);
        ...
        std::string logical_port_ctr = reinterpret_cast<char*>(vlanmap_key->logical_port_id);
...
TEST(odcdriver,  test_fetch_conf_vlanmap_resp_max_vlanid) {
  ...
  EXPECT_EQ(DRVAPI_RESPONSE_FAILURE, odc_vbr_vlanmap.validate_vlan_exist(key_vlan_map, val_vlan_map, ctr, is_switch_exist, port_id));
...
TEST(odcdriver,  test_validate_vlan_switch_requested_incorrect_vlan_parse) {
  ...
  EXPECT_EQ(DRVAPI_RESPONSE_FAILURE, odc_vbr_vlanmap.validate_vlan_exist(key_vlan_map, val_vlan_map, odc_ctr, is_switch_exist, port_id));
  EXPECT_EQ(is_switch_exist, PFC_FALSE);
  if (ctr != NULL) {
    delete ctr;
    ctr = NULL;
...
TEST(odcdriver,  test_validate_vlan_switch_requested_vlan_diff_sw_exist) {
  ...
  unc::odcdriver::OdcVbrVlanMapCommand odc_vbr_vlanmap;
  EXPECT_EQ(DRVAPI_RESPONSE_SUCCESS, odc_vbr_vlanmap.validate_vlan_exist(key_vlan_map, val_vlan_map, odc_ctr, is_switch_exist, port_id));
