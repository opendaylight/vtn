...
def validate_vbr_at_controller(vtn_blockname, vbr_blockname,controller_blockname, presence="yes",position=0):
    ...
    url='http://'+test_controller_ipaddr+':'+test_controller_port+controller_url_part+test_vtn_url+'/'+test_vtn_name+test_vbr_url
    print url
    r = requests.get(url,headers=controller_headers,auth=('admin','admin'))
    print r.status_code
...
def test_vtn_multi_vbr_test():
    ...
    retval=validate_vbr_at_controller('VtnOne','VbrTwo','ControllerFirst',position=1)
    if retval != 0:
...
def test_vtn_vbr_audit_1():
    ...
    print """TEST 4 : create VTN and VBR when controller is up
...
def test_vtn_vbr_audit_2():
  ...
  # Delay for AUDIT
    ...
    print """TEST 5 : create VTN and VBR when controller is down
