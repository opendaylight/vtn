...
def test_vtermif_flowfilter_1():
  ...
  retval=vtn_vbr.validate_vbr_at_controller('VtnOne','VbrThree','ControllerFirst', presence = 'yes', position = 1)
  if retval != 0:
    print "VBR Validate Failed"
    exit(1)
  
  retval=vbrif_portmap.validate_vbrif_at_controller('VtnOne','VbrThree','VbrIfThree','ControllerFirst', presence = 'yes', position = 0)
  if retval != 0:
    print "After Create VBRIF Validate Failed"
    exit(1)
  ...
  retval=flowfilter.update_flowfilter_entry('VtnOne|VbrOne|VbrIfOne|VTermIfOne', 'FlowfilterOne', 'UpdateFlowfilter')
  if retval != 0:
    print "VBRFIFlowFilterEntry Update Failed"
    exit(1)
  
  retval=flowfilter.validate_flowfilter_entry('VtnOne|VbrOne|VbrIfOne|VTermIfOne', 'FlowfilterOne|UpdateFlowfilter', presence='yes', position=0)
  if retval != 0:
