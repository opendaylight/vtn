...
def validate_vbrif_portmap_at_controller(vtn_blockname, vbr_blockname, vbrif_blockname,
    ...
    url='http://'+test_controller_ipaddr+':'+test_controller_port+controller_url_part+test_vtn_url+test_vbr_url+test_vbrif_url+'/'+test_vbrif_name+'/portmap'
    print url
    r = requests.get(url,headers=controller_headers,auth=('admin','admin'))
    ...
    if vtn_content != test_vbrif_node_id:
        if presence == "yes":
            return 1
        else:
            return 0
...
def test_multi_vbrif():
  ...
  # Delay for AUDIT
    ...
    retval=validate_vbrif_at_controller('VtnOne','VbrOne','VbrIfTwo','ControllerFirst',presence="no",position=1)
    if retval != 0:
...
def test_vtn_multi_vbr_vbrif():
  ...
  # Delay for AUDIT
    ...
    retval=validate_vbrif_at_controller('VtnOne','VbrTwo','VbrIfTwo','ControllerFirst',position=1)
    if retval != 0:
        print "VBR2->VBRIF2 Validate Failed"
        exit(1)
    ...
    retval=validate_vbrif_at_controller('VtnOne','VbrOne','VbrIfThree','ControllerFirst',presence="no",position=1)
    if retval != 0:
    ...
    retval=validate_vbrif_at_controller('VtnOne','VbrTwo','VbrIfTwo','ControllerFirst',presence="no",position=1)
    if retval != 0:
        print "VBR2->VBRIF2 Validate Failed"
        exit(1)
    retval = vtn_vbr.delete_vbr('VtnOne','VbrOne')
...
def test_vtn_multi_vbr_vbrif_portmap():
  ...
  # Delay for AUDIT
    ...
    retval=validate_vbrif_portmap_at_controller('VtnOne','VbrOne','VbrIfTwo','ControllerFirst',presence="yes",position=1);
    if retval != 0:
    ...
    retval=validate_vbrif_portmap_at_controller('VtnOne','VbrOne','VbrIfTwo','ControllerFirst',presence="no",position=1);
    if retval != 0:
...
def test_vtn_vbr_multiple_vbrif_portmap():
  ...
  # Delay for AUDIT
    ...
    retval=validate_vbrif_at_controller('VtnOne','VbrTwo','VbrIfThree','ControllerFirst',position=1)
    if retval != 0:
    ...
    retval=validate_vbrif_portmap_at_controller('VtnOne','VbrTwo','VbrIfThree','ControllerFirst',presence="yes",position=1);
    if retval != 0:
    ...
    retval=validate_vbrif_at_controller('VtnOne','VbrTwo','VbrIfFour','ControllerFirst',presence="no",position=1)
    if retval != 0:
...
def test_audit_vtn_vbr_vbrif():
    ...
    print "TEST 13 : Test Audit VTenant with one VBridge one VBRIF"
  # Delay for AUDIT
    time.sleep(15)
    retval=vtn_vbr.create_vtn('VtnOne')
    if retval != 0:
...
def test_audit_multi_vbrif():
    ...
    retval = controller.add_controller_ex('ControllerFirst')
    if retval != 0:
        print "TEST 14 :Controller Create Failed"
        exit(1)

    print "TEST 14 : Test audit One vtn and one VBridge with Two Interfaces"
  # Delay for AUDIT
    time.sleep(15)
    retval=vtn_vbr.create_vtn('VtnOne')
    if retval != 0:
...
def test_audit_vtn_multi_vbr_vbrif():
    ...
    retval = controller.add_controller_ex('ControllerFirst')
    if retval != 0:
        print "TEST 15 :Controller Create Failed"
        exit(1)

    print "TEST 15 : Audit One vtn and Two VBridges with Two Interfaces each"
  # Delay for AUDIT
    time.sleep(15)
    retval=vtn_vbr.create_vtn('VtnOne')
    if retval != 0:
...
def test_audit_vtn_vbr_vbrif_portmap():
    ...
    retval = controller.add_controller_ex('ControllerFirst')
    if retval != 0:
        print "TEST 16 :Controller Create Failed"
        exit(1)

    print "TEST 16 : Test Audit with VTenant one VBridge one VBRIF and One PORTMAP"
  # Delay for AUDIT
    time.sleep(15)
    retval=vtn_vbr.create_vtn('VtnOne')
    if retval != 0:
...
def test_audit_vtn_multi_vbr_vbrif_portmap():
    ...
    retval = controller.add_controller_ex('ControllerFirst')
    if retval != 0:
        print "TEST 17 :Controller Create Failed"
        exit(1)

    print "TEST 17 : VTenant with one VBridge Two VBRIF and One PORTMAP each"
  # Delay for AUDIT
    time.sleep(15)
    retval=vtn_vbr.create_vtn('VtnOne')
    if retval != 0:
...
def test_audit_vtn_vbr_multi_vbrif_portmap():
    ...
    retval = controller.add_controller_ex('ControllerFirst')
    if retval != 0:
        print "TEST 18 :Controller Create Failed"
        exit(1)

    print "TEST 18 : VTenant with one VBridge Two VBRIF1/VBRIF2 and One PORTMAP in VBRIF1"
  # Delay for AUDIT
    time.sleep(15)
    retval=vtn_vbr.create_vtn('VtnOne')
    if retval != 0:
