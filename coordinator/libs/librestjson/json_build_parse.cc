...
json_object* JsonBuildParse::CreateJsonObj() {
  json_object* jsonobj = json_object_new_object();
  return jsonobj;
}
const char* JsonBuildParse::JsonObjToJsonString(json_object* jobj) {
  if (json_object_is_type(jobj, json_type_null)) {
    return NULL;
  }
  return json_object_to_json_string(jobj);
}
int JsonBuildParse::GetType(json_object* jobj, const std::string &key) {
  if (json_object_is_type(jobj, json_type_null)) {
    return FAILURE;
  }
...
int JsonBuildParse::GetArrayLength(json_object* jobj, const std::string &key) {
  if (json_object_is_type(jobj, json_type_null)) {
    return FAILURE;
  }
  json_object *jobj_value = json_object_object_get(jobj, key.c_str());
