...
package com.vtn.app.run.config.rest.client;
...
import com.vtn.app.run.config.rest.output.format.beans.VBridgeBean;
import com.vtn.app.run.config.rest.output.format.beans.VBridgeInterfaceBean;
import com.vtn.app.run.config.rest.parser.CRUDImpl;
import com.vtn.app.run.config.rest.parser.Parser;
import com.vtn.app.run.config.rest.parser.RestRequest;
import com.vtn.app.run.config.rest.response.beans.ContainerPathMapNorthBoundList;
import com.vtn.app.run.config.rest.response.beans.DataFlowList;
import com.vtn.app.run.config.rest.response.beans.FlowconditionList;
import com.vtn.app.run.config.rest.response.beans.Index;
...
import com.vtn.app.run.config.rest.response.beans.PathPoliciesIndex;
import com.vtn.app.run.config.rest.response.beans.PathPolicy;
import com.vtn.app.run.config.rest.response.beans.VBridgeInterface;
import com.vtn.app.run.config.rest.response.beans.VBridgeInterfaceList;
import com.vtn.app.run.config.rest.response.beans.VBridgeNorthBound;
import com.vtn.app.run.config.rest.response.beans.VBridgeNorthBoundList;
import com.vtn.app.run.config.rest.response.beans.VBridgePortMap;
import com.vtn.app.run.config.rest.response.beans.VTNConfiguration;
import com.vtn.app.run.config.rest.response.beans.VTNConfigurationBean;
import com.vtn.app.run.config.rest.response.beans.VTNManagerVersion;
import com.vtn.app.run.config.rest.response.beans.VTenantNorthBound;
import com.vtn.app.run.config.rest.response.beans.VTenantNorthBoundList;
...
public class RESTClient {
     ...
     * @throws Exception
     */
    public RESTClient(String serverIp, String serverPort) throws Exception {
        ...
        client = new CRUDImpl(APPLICATION_TYPE.JSON);
        parser = new Parser();
    }
     ...
     *
     * @throws Exception
     *             Throws Exception if unable to construct the Rest
     *             Request
     */
    ...
    public RestRequest getRestRequest(Object obj, Map<String, String> paramMap)
        ...
        if (restUrl != null) {
            ...
            throw new Exception("Incompatible Object (Unable to find '"    + RestURL.class.getName() + "' annotation)");
        }
    }
    ...
    private Object parseResponse(Object target, RestRequest request)
        ...
        if (request.getResponse() != null && !request.getResponse().equals("")
            ...
            if (request.getResponse().startsWith("[")) {
                ...
                //return parser.setAnnotatedFields(target, jsonObject);
            }
        } else {
            return target;
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Please Enter the VTN Server IP Address:");
            String ipAddress = scanner.nextLine();

            System.out.println("Please Enter teh Port to communicate:");
            ...
            RESTClient handler = new RESTClient(ipAddress, port);
            ...
            VTenantNorthBoundList vtnList = (VTenantNorthBoundList)handler.get(new VTenantNorthBoundList());
            ...
            if (vtnList.getVtns().size() > 0) {
                for (VTenantNorthBound vtn : vtnList.getVtns()) {
                    ...
                    System.out.println(" == == == == == == == == == == == == == == = RUNNING CONFIGURATION STARTED == == == == == == == == == == == == == == == == == == == == == == == == =  = ");
            ...
            if (policyIndex.getIntegers().size() > 0) {
                for (Index index  : policyIndex.getIntegers()) {
                    ...
                    System.out.println("PATHPOLICY:" + pathPolicy);
                }
            }
            ...
            conf.setRunConfig(bean);

            handler.jsonFormating(conf);
        } catch (Exception e) {
     ...
     * @param jsonVal
     */
    private  void jsonFormating(Object jsonVal) {
        Object json  = jsonVal;
        try {
            if (!(json.equals(null))) {
