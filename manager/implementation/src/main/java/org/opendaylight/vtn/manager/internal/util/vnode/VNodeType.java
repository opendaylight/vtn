...
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_VTN;
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_VBRIDGE;
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_VTERMINAL;
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_VINTERFACE;
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_VLANMAP;
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_MACMAP;
import static org.opendaylight.vtn.manager.internal.util.vnode.VNodeIdentifier.TYPEBIT_MACMAP_HOST;
...
public enum VNodeType implements VNodeIdentifierFactory {
     ...
     */
    public VnodeName checkName(String name, boolean find) throws RpcException {
        try {
            return MiscUtils.checkName(description, name);
        } catch (RpcException e) {
            RpcException cause = e;
            if (find && e.getErrorTag() == RpcErrorTag.BAD_ELEMENT) {
                // The specified virtual node should not be present because
                // the given name is invalid.
                cause = getNotFoundException(name, e);
            }
            throw cause;
        }
    }
     ...
     *               the virtual node.
     * @return  A string in {@code vname}.
     * @throws RpcException  The specified name is invalid.
     */
    public String checkName(VnodeName vname) throws RpcException {
        return MiscUtils.checkName(description, vname);
    }

    /**
     * Return a new {@link RpcException} that indicates the virtual node
     * is not present.
     *
     * @param name  The name of the virtual node.
     * @return  An {@link RpcException}.
     */
    public RpcException getNotFoundException(String name) {
     ...
     * @return  An {@link RpcException}.
     */
    public RpcException getNotFoundException(String name, Throwable cause) {
        String msg = name + ": " + description + " does not exist.";
        return (cause == null)
            ? RpcException.getNotFoundException(msg)
            : RpcException.getNotFoundException(msg, cause);
    }
     ...
     * @return  An {@link RpcException}.
     */
    public RpcException getDataExistsException(String name) {
        String msg = name + ": " + description + " already exists.";
