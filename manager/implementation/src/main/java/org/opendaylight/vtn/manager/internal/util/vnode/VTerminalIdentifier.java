...
public final class VTerminalIdentifier extends BridgeIdentifier<Vterminal> {
     ...
     */
    public static VTerminalIdentifier create(
        String tname, String bname, boolean find) throws RpcException {
        VnodeName vtnName = VNodeType.VTN.checkName(tname, true);
        VnodeName vtermName = VNodeType.VTERMINAL.checkName(bname, find);
        return new VTerminalIdentifier(vtnName, vtermName);
    }

    /**
     * Create a new identifier.
     *
     * @param path  A {@link VterminalPathFields} instance.
     * @param find  {@code true} indicates the given name is used for
     *              finding existing virtual node.
     * @return  A {@link VTerminalIdentifier} instance.
     * @throws RpcException  Invalid name is specified.
     */
    public static VTerminalIdentifier create(
        VterminalPathFields path, boolean find) throws RpcException {
        return create(path.getTenantName(), path.getTerminalName(), find);
    }
