...
public class TestUseVTNManagerBase extends TestBase {
    ...
    protected void setupVTNManagerForRemoteTaskTest(long timeout) {
        setupVTNManagerForRemoteTaskTest(GlobalConstants.STARTUPHOME.toString(),
                               CONFIG_FILE_NAME, timeout);
    }

    protected void setupVTNManagerForRemoteTaskTest(String dir, String fileName, long timeout) {
        FileWriter gWriter;
        File gIniFile = new File(dir, fileName);
        try {
            gWriter = new FileWriter(gIniFile);
            gWriter.write("remoteFlowModTimeout=" + timeout);
            gWriter.close();
        } catch (IOException e) {
            unexpected(e);
        }
