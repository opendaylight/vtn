...
public class OVSDBEventHandler  {
    ...
    public void nodeAdded(Node node, DataObject resourceAugmentationData ) {
        LOG.debug("nodeAdded Method invoked" );
        try {
            createInternalNetworkForNeutron(node);
        } catch (Exception e) {
            LOG.info("Exception occurred while Bridge Creation", e);
        }
    }
    ...
    public void nodeRemoved(Node node) {
        LOG.info("nodeRemoved() - {}", node.toString());
    }
    ...
    public void createInternalNetworkForNeutron(Node node) throws Exception {
        LOG.debug("inside create Internal Network" );
        getSystemProperties(node);
        LOG.info("createInternalNetworkForNeutron() - node ={}, integration bridge ={}", node.toString());
    ...
    public void getSystemProperties(Node node) {
        ...
        try {
            ...
            prop.load(in);
            LOG.info("loaded Integration bridge Configuring details from vtn.ini");
        } catch (Exception e) {
            LOG.info("Exception occurred while reading SystemProperties", e);
        } finally {
            if (in != null) {
                try {
                    in.close();
        ...
        LOG.info("System properties values : {},{},{},{}:",
                   ...
                   try {
                          if (!isBridgeOnOvsdbNode(node, integrationBridgeName)) {
                             ...
                             addPortToBridge(node,integrationBridgeName,portname);
                                ...
                                LOG.info("Bridge Already exists in the given Node");
                             }
                   } catch (Exception e) {
                       LOG.info("Exception occurred while Adding Bridge", e);
                       }
                 }
    }
...
/**
    ...
    private ConnectionInfo getConnectionInfo(Node ovsdbNode) {
        LOG.info("ConnectionInfo Node {}" ,ovsdbNode);
        ConnectionInfo connectionInfo = null;
        OvsdbNodeAugmentation ovsdbNodeAugmentation = extractOvsdbNode(ovsdbNode);
         LOG.info("OvsdbNodeAugmentation  Node {}" ,ovsdbNodeAugmentation);
        if (ovsdbNodeAugmentation != null) {
            connectionInfo = ovsdbNodeAugmentation.getConnectionInfo();
             LOG.info("return connection info  Node {}" ,connectionInfo);
        }
        return connectionInfo;
    }
            ...
            if (ovsdbNodeAugmentation != null) {
                ...
                if (connectionInfo != null && connectionInfo.getLocalIp() != null) {
                    controllerIpStr = new String(connectionInfo.getLocalIp().getValue());
                           ...
                           LOG.info("Ovsdb Node does not contains connection info : {}", node);
                }
            }
            if (controllerIpStr != null) {
                LOG.info("Target OpenFlow Controller found : {}", controllerIpStr);
    ...
    private boolean addPortToBridge (Node node, String bridgeName, String portName) throws Exception {
        ...
        if (extractTerminationPointAugmentation(node, portName) == null) {
            ...
            LOG.info("addPortToBridge: node: {}, bridge: {}, portname: {} status: not_needed",
                    node.getNodeId().getValue(), bridgeName, portName);
        }
    ...
    private void setManagedByForBridge(OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder,
        ...
        LOG.info("setManagedByForBridge: connectionNodePath: {}", connectionNodePath);
